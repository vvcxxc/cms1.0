...
//饼图内容显示
function PieLabelFun(DimensionShow, PercentageShow, ValueShow, ShowOutter, Unit, showCustom) {
    var Lableformatter
    ...
    if (PercentageShow == true) {//百分比
        if (showCustom) {
            Lableformatter = function (e) {
                let newStr = PieLabelConter(e)
                if (ShowOutter) {
                    return '' + /* e.percent */ e.data.value + '%' + ' ' + Unit
                } else {
                    return + /* e.percent */ e.data.value + '% ' + Unit
                }
            }
    ...
    if (ValueShow == true && PercentageShow == true) {//数值+百分比
        if (showCustom) {
            Lableformatter = function (e) {
                var newStr = PieLabelConter(e)
                if (ShowOutter) {
                    return  /* e.value */  e.data.originValue + '\n' + '' + /* e.percent */  e.data.value + '%' + ' ' + Unit
                } else {
                    return  /* e.value */ e.data.originValue + '\n' + /* e.percent */ e.data.value + '%' + ' ' + Unit
                }
            }
    ...
    if (changedata.hasOwnProperty('PieChart')) {
        if (changedata.PieChart.length !== 0) {
            for (let i = 0; i < changedata.PieChart.length; i++) {
                ...
                var Lableformatter
                Lableformatter = PieLabelFun(data.Label.DimensionShow, data.Label.PercentageShow, data.Label.ValueShow, ShowOutter, Unit, data.showCustom)
                ...
                allchartlist.push([Chart1, changedata.PieChart[i].name, data.Label.DimensionShow, data.Label.PercentageShow, data.Label.ValueShow, ShowOutter, Unit, data.showCustom]);
                const piecs = Chart1
    ...
    if (changedata.hasOwnProperty('BarChart')) {
        if (changedata.BarChart.length !== 0) {
            for (let i = 0; i < changedata.BarChart.length; i++) {
                ...
                console.log("data.DrawContent",data.DrawContent)
                var gridLeft = data.DrawContent.LeftMargin
                var gridRight = data.DrawContent.RightMargin
                var gridBottom = data.DrawContent.BottomMargin
                var gridTop = data.DrawContent.TopMargin
                ...
                // 右侧y轴
                if (!!data.rightYAxis && data.rightYAxis.Show) {
                    rightYAxis.Yname = data.rightYAxis.Name
                    rightYAxis.Adaptive = data.rightYAxis.Adaptive
                    rightYAxis.YAxisMax = data.rightYAxis.Max
                    rightYAxis.YAxisMin = data.rightYAxis.Min
                    rightYAxis.YAxisMax1 = data.rightYAxis.Max
                    rightYAxis.YAxisMin1 = data.rightYAxis.Min
                    rightYAxis.YsplitLine = data.rightYAxis.SplitLineShow
                    rightYAxis.YfontSize = data.rightYAxis.FontSize
                    rightYAxis.IsCustomSegmentation = data.rightYAxis.IsCustomSegmentation
                    rightYAxis.CustomSegmentation = data.rightYAxis.CustomSegmentation
                    rightYAxis.Ycolor = colorRgba(data.rightYAxis.Color.HtmlColor)
                    rightYAxis.Unit = data.rightYAxis.Unit == null ? '' : data.rightYAxis.Unit
                    rightYAxis.SplitLineFontWeight = data.rightYAxis.SplitLineFontWeight
                    rightYAxis.SplitLineWidth = data.rightYAxis.SplitLineWidth
                    rightYAxis.SplitLineColor = colorRgba(data.rightYAxis.SplitLineColor.HtmlColor)
                    rightYAxis.YFontFamily = data.rightYAxis.FontFamily
                    rightYAxis.ShowSeparator = data.rightYAxis.ShowSeparator
                }
                ...
                for (let v = 0; v < VariablesData.length; v++) {
                    ...
                    var value2 = {
                        ...
                        markLine: '',
                        barGap: VariablesData[v].BarGap + '%',
                        yAxisIndex: VariablesData[v].yAxisIndex
                    }
                ...
                if (!!data.rightYAxis && data.rightYAxis.Show) {
                    BarChartOption.yAxis.push(
                        {
                            name: rightYAxis.Yname,
                            IsTime: IsTime,
                            IsOrigin: IsOrigin,
                            IsDuidie: IsDuidie,
                            Adaptive: rightYAxis.Adaptive,
                            position: 'right',
                            offset: 0,
                            splitNumber: !rightYAxis.IsCustomSegmentation ? 0 : rightYAxis.CustomSegmentation,
                            axisLabel: {
                                formatter: "{value} " + rightYAxis.Unit,
                                color: rightYAxis.Ycolor,
                                fontFamily: rightYAxis.YFontFamily,
                                fontWeight: 400,
                                fontSize: rightYAxis.YfontSize,
                                textStyle: {
                                    fontWeight: 400
                                }
                            },
                            ...
                            min: rightYAxis.YAxisMin,
                            max: rightYAxis.YAxisMax,
                            YAxisMax1: rightYAxis.YAxisMax1,
                            YAxisMin1: rightYAxis.YAxisMin1,
                            nameTextStyle: {
                                fontSize: rightYAxis.YfontSize,
                                color: rightYAxis.Ycolor
                            },
                            ...
                            splitLine: {
                                show: rightYAxis.ShowSeparator,
                                "lineStyle": {
                                    type: 'dashed',
                                    "color": data.ThemeType == "Dark" ? "rgba(210,210,210,0.49)" : "rgba(210,210,210,0.9)"
                                },
                                color: rightYAxis.SplitLineColor
                            }
                        }
                    )
                }
        ...
        for (let j = 0; j < changedata.LineBarChart.length; j++) {
            ...
            for (var i = 0; i < allchartlist.length; i++) {
                if (allchartlist[i][1] == changedata.LineBarChart[j].name) {
                    ...
                    // 	})
                    console.log("BarChartOption", JSON.stringify(optionli2))   // }
        ...
        for (let j = 0; j < changedata.PieChart.length; j++) {
            for (var i = 0; i < allchartlist.length; i++) {
                if (allchartlist[i][1] == changedata.PieChart[j].name) {
                    ...
                    Lableformatter = PieLabelFun(allchartlist[i][2], allchartlist[i][3], allchartlist[i][4], allchartlist[i][5], allchartlist[i][6], allchartlist[i][7])
                    if (changedata.PieChart[j].hasOwnProperty('YDataCollection')) {
                        ...
                        if (changedata.PieChart[j].YDataCollection.length) {
                            for (let a = 0; a < changedata.PieChart[j].YDataCollection.length; a++) {
                                ...
                                optionli.series[0].data[index].originValue = changedata.PieChart[j].YDataCollection[a].originValue
                    ...
                    for (let f1 = 0; f1 < allchartlist.length; f1++) {
                        var listName = allchartlist[i][1] + 'pie'
                        if (listName == allchartlist[f1][1]) {
                            if (optionli2.backgroundColor.substring(optionli2.backgroundColor.length - 2) == '00') {
                                optionli2.series[0].data.forEach((item) => {
...
function changeCheckbox(e, name, id) {
    let dataCount = document.getElementById(`${name}Data`)

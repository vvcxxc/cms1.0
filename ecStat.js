	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
	    ...
	    function computeBins(data, threshold) {
	        ...
	        var rangeArray = range(
                    ...
                    +((Math.ceil(minValue / step) * step).toFixed(2)),
	                +((Math.floor(maxValue / step) * step).toFixed(2)),
	                step,
	                precision
	            );
	        ...
	        var data = map(bins, function (bin) {
	            ...
	            return [+((bin.x0 + bin.x1) / 2).toFixed(2), bin.sample.length];
	        });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
	    ...
	    return function (start, end, step, precision) {
	        ...
	        var n = Math.ceil(((end - start) / step).toFixed(2));
	        var range = new Array(n + 1);
	        for (var i = 0; i < n + 1; i++) {
	            //range[i] = +(start + i * step).toFixed(precision);
	            range[i] = +(start + i * step).toFixed(2);
	        }
	        return range;
	    };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
	    ...
	    return function (start, stop, count) {
	        ...
	        return +((stop >= start ? step1 : -step1).toFixed(2));

...
<div class="customerPage" style="position:relative;overflow:auto;margin-bottom:0px;margin-top:100px;" :style="[{height:nowoverflow},{width:newoverflow}]">
  <div class='btablecover'>
    <div class='btabletop'>导出进度</div>
    <div class='btableconter'>
      <span class='btableconterword'>正在导出</span>
 ...
 <div  v-loading="this.$store.state.isShow" class="big_box report" v-else>
    <LineVue1 :dataId="dataId" :dae="data" :key="1+componentKey"></LineVue1>
    <RectangleVue2 :dataId="dataId" :dae="data" :key="2+componentKey"></RectangleVue2>
    <EllipseVue3 :dataId="dataId" :dae="data" :key="3+componentKey"></EllipseVue3>
    <StaticImage4 :dataId="dataId" :name="name" :dae="data" :key="4+componentKey"></StaticImage4>
...
export default {  
    ...
    watch:{
      screen(val){
          let w = window.screen.width/1920;
             let a = $('.v-toolbar').height()*$('.v-toolbar')[0].style.zoom+10+$('.v-footer').height()*$('.v-footer')[0].style.zoom
          let h = (window.screen.height-a)/952;
 ...
 $('.report').css({
           ...
           setTimeout(()=>{
             ...
             if(parseFloat(JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Height)>(window.screen.height-120)){
              width-=17
            }
              $('.scalediv1').css({
            height:952+'px',
 ...
 $('.report').css({
            transform:`scale(${h},${h})`,
                 ...
                 if(this.$store.state.screentype == 2||this.$store.state.screentype == 4||this.$store.state.screentype == 1){
      
      if( JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Width>window.screen.width){
            let a = $('.v-toolbar').height()*$('.v-toolbar')[0].style.zoom+10+$('.v-footer').height()*$('.v-footer')[0].style.zoom+'px'
            this.nowoverflow = `calc(100% - ${a})`
         }else{
             let a = $('.v-toolbar').height()*$('.v-toolbar')[0].style.zoom+10+$('.v-footer').height()*$('.v-footer')[0].style.zoom-20+'px'
            this.nowoverflow = `calc(100% - ${a})`
         }
    }
     $('.customerPage').css({
      ...
      pageHeight(n,o){   //监听浏览器 改变可视页面大小使滚动条展示 
      ...
      "$store.state.setTiemValue": {  //监听筛选控件查询值
          deep: true,
          handler:function(n, o){
            this.$refs.table.search("默认查询");
             var option = this.$refs.querybutton25.timingSearch(this.$store.state.setTiemValue.ViewName,this.$store.state.setTiemValue.Data)
            option.then(x=>window.$.connection.subchart.server.query(this.$store.state.setTiemValue.ViewName,this.$store.state.setTiemValue.Data,x,localStorage.getItem('currentLang')));
          }
      },
      "$store.state.ReceivedAlarms": {  //监听报警控件信息
          deep: true,
          handler:function(n, o){
            this.$refs.Alarm14.axioImg2(this.$store.state.ReceivedAlarms)
          }
      },
      "$store.state.websocketsShow": {  //是否开始连接websockets
          deep: true,
          handler:function(n, o){
       ...
       listData(n,o){  //监听路由发生改变
          clearInterval(this.scriptTime)
          if(this.$store.state.websocketsShow != null){
            if(window.$.connection.chart){       //调用服务器端方法 离开视图
            try{
                window.$.connection.subchart.server.leaveView(`${this.name}`) //停止
              }catch{
                console.log('11111')
              }
            }
          ...
          self.init()
           if(this.$store.state.websocketsShow != null){
             if(window.$.connection.chart){      //调用服务器端方法 加入视图
    ...
    destroyed(){
      if(window.$.connection.chart){        //调用服务器端方法 离开视图
        window.$.connection.subchart.server.leaveView(`${this.name}`)
      }
      clearInterval(this.scriptTime)
    ...
    console.log("触发1111")
      ...
      // this.pageHTime = setInterval(()=>{  //实时刷新浏览器可视页面大小
      //   try {
 ...
 $('.report').css({
           ...
           let height = window.screen.height-130
             if(parseFloat(JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Width)>(window.screen.width)){
              height-=17
              console.log("saddsadsa")
            }
 ...
 $('.report').css({
    ...
    methods: {
     ...
     search(a,num,i1){   //组件25查询事件
       this.$refs.table.search(a,num,i1);
     },
     exportdata(a,num){  //组件26导出事件
   ...
   canvasSizeFun(type,heightType){
    ...
    dataFun(text){
        ...
        if(widthWV>800){      //首次小于1280执行之后小于1280不执行
          this.isMinW = true
        }
        if(text != '跳转'){
          if(!this.isMinW){
...
//       if( JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Width>screenWV){
//              this.nowoverflow = 'calc(100% - 130px)'
//          }else{
//             this.nowoverflow = 'calc(100% - 110px)'
//          }
//     }
//     if(this.$store.state.screentype !== 1){
//  if( JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Height>(screenHv-120)){
//      screenWV-=17
//         }
      
//      if( JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Width>screenWV){
//       screenHv-=17
//       $('.report')[0].style.Height = screenHv +'px'
//          }  
//     }else{
//        if( JSON.parse(JSON.stringify(this.JsonData)).Controls.PropertyList.Width>screenWV){
//       screenHv-=2
...
//     }
        ...
        //画布高度比例
        //画布宽度比例
        if(Number(originalData.Controls.PropertyList.Width) < Number(screenWV)){   //小画布
            windowType = 'Small'
       ...
       var value = scriptData
              ...
              if(name == 'line'){    //直线
                ...
                if(value[i].Property == 'Stroke'){
                 ...
                 let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`)[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`)[0].style.width);
    ...
    if(value[j].Property == 'Move'){
            ...
            if(value[i].Property == 'Move'){
              ...
              if(name == 'cornerbutton' || name == 'pressbutton' || name == 'tabmodulebutton' || name == 'popupbutton'){
                 ...
                 if(value[i].Property == 'background'){                                                //控制按钮、点动、弹窗、跳转
      ...
      if(value[i].Property == 'Move'){
              ...
              if(name == 'switch'){      //开关
                if(value[i].Property == 'background'){     //开关不改字体颜色和背景颜色
                  if($(`.${value[i].Control}ipt`)[0].checked){
                    $(`.${value[i].Control}`).find('#switch-area').css('background',value[i].Value) 
                  }
                }
                 if(value[i].Property == 'font-family'){  
        ...
        if(value[i].Property == 'Move'){
              ...
              if(name==="matrixgridset" || name === "matrixgrid"){    //单点矩阵、矩阵设定
                if(value[i].Property == 'opacity'){
                  $(`.${value[i].Control}`).css(value[i].Property,value[i].Value)
                }
                if(value[i].Property == 'background'){
                  for(let j=0;j<$(`.${value[i].Control}`).find('.matrixgrid-cell').length;j++){
                    if(!$(`.${value[i].Control}`).find(".matrixgrid-cell:eq("+ j +")").attr("data-change")){
                      $(`.${value[i].Control}`).find(".matrixgrid-cell:eq("+ j +")").css(value[i].Property,value[i].Value)
                    }
                  }
                }
                if(value[i].Property == 'font-family'){
                  ...
                  $(`.${value[i].Control}`).find('.matrixgrid-set').css(value[i].Property,value[i].Value)
                }
                if(value[i].Property == 'font-size'){
                  $(`.${value[i].Control}`).find('.matrixgrid-set').css(value[i].Property,value[i].Value)
                }
                if(value[i].Property == 'color'){
                  $(`.${value[i].Control}`).find(".matrixgrid-set input").css("-webkit-text-fill-color",value[i].Value)
                }
                if(value[i].Property == 'font-weight'){
                  if(value[i].Value == 'none'){
                    value[i].Value = 400
                ...
                if(value[i].Property == 'StrokeThickness'){
                  $(`.${value[i].Control}`).css('padding',value[i].Value + 'px')
                }
                if(value[i].Property == 'IsEnabled'){
                  $(`.${value[i].Control}`).find('.matrixgrid-set input').attr("disabled",true)
                }
                let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`)[0].style.height);
                let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`)[0].style.width);
                let moveleft = parseFloat($(`.${value[i].Control}`)[0].style.left);
              ...
              if(name == 'datatextbox' || name =="rwtextbox"){    //数值输入、读写框
                ...
                if(value[i].Property == 'StrokeThickness'){
                 ...
                 let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`).parents('.contenBox')[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`).parents('.contenBox')[0].style.width);
        ...
        if(value[i].Property == 'Move'){
                ...
                if(movehtml){
                  console.log("sadasdasd",$(`.${value[i].Control}`).parents('.contenBox')[0])
                  if(!$(`.${value[i].Control}`).parents('.contenBox').is(':animated')){
                     movehtml=`$('.`+value[i].Control+`').parents('.contenBox')`+movehtml
                     eval(movehtml)
                  }
            ...
            if(name == 'datatextblock'){   //数值显示
               var Th = parseInt($(`.${value[i].Control}`).parents('.DataTextBlock121aa1').css('height'))
               if(value[i].Property == 'opacity'){
                 $(`.${value[i].Control}`).parents('.DataTextBlock121aa1').css(value[i].Property,value[i].Value)
               }
               if(value[i].Property == 'background'){
                 $(`.${value[i].Control}`).css(value[i].Property,value[i].Value)
              }
               if(value[i].Property == 'font-size'){
                ...
                if(value[i].Property == 'Stroke'){
                $(`.${value[i].Control}`).parents('.DataTextBlock121aa1').css('background',value[i].Value)
              }
              if(value[i].Property == 'StrokeThickness'){
                $(`.${value[i].Control}`).parents('.DataTextBlock121aa1').css('padding',value[i].Value + 'px')
                $(`.${value[i].Control}`).parents('.DataTextBlock121aa1').css('line-height',Th - (value[i].Value * 2) + 'px')
              }
                  let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`).parents('.DataTextBlock121aa1')[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`).parents('.DataTextBlock121aa1')[0].style.width);
                 let moveleft = parseFloat($(`.${value[i].Control}`).parents('.DataTextBlock121aa1')[0].style.left);
                 let movetop = parseFloat($(`.${value[i].Control}`).parents('.DataTextBlock121aa1')[0].style.top);
                 let moveindex = -1
                 let movetime = 0
                 let movetype = 0
                 let movehtml = ``
        ...
        if(value[i].Property == 'Move'){
                ...
                if(movehtml){
                  if(!$(`.${value[i].Control}`).parents('.DataTextBlock121aa1').is(':animated')){
                     movehtml=`$('.`+value[i].Control+`').parents('.DataTextBlock121aa1')`+movehtml
                     eval(movehtml)
                  }
            ...
            if(name == 'ellipselamp' || name=='commonlamp'){  //圆形
         ...
         if(value[i].Property == 'Move'){
          ...
          if(value[i].Property == 'Move'){
            ...
            if(name == 'outputbutton' || name == 'querybutton'){   //查询、导出
               if(value[i].Property == 'background'){
                    $(`.${value[i].Control}`).find('.conten').css('background',value[i].Value)
                }
                if(value[i].Property == 'text-decoration'){
                ...
                if(value[i].Property == 'Text'){
                 ...
                 if(value[i].Property == 'font-weight' || value[i].Property == 'font-family' || value[i].Property == 'font-size' || value[i].Property == 'color'){
           ...
           if(value[i].Property == 'Move'){
            ...
            if(name == 'Combobox'){  //下拉框样式
              ...
              || value[i].Property == 'font-weight' || value[i].Property == 'font-size' || value[i].Property == 'text-decoration'){
                 ...
                 $(`.${value[i].Control}`).find('.el-input__inner').css(value[i].Property,value[i].Value)
              ...
              if(value[i].Property == 'opacity'){
                $(`.${value[i].Control}`).css('opacity',value[i].Value)
              }
                 let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`)[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`)[0].style.width);
                 let moveleft = parseFloat($(`.${value[i].Control}`)[0].style.left);
            ...
            if(name == "DateTimePicker"){  //时间控件样式
              ...
              || value[i].Property == 'text-decoration' || value[i].Property == 'font-weight' || value[i].Property == 'font-family'){
                ...
                $(`.${value[i].Control}picker`).find('.el-input__inner').css(value[i].Property,value[i].Value)
              }
              if(value[i].Property == 'Stroke'){
                $(`.${value[i].Control}`).css('background',value[i].Value)
              }
       ...
       for(let j = 0;j<value.length;j++){
            ...
            if(name == 'textimport'){ //数值输入
               if(value[i].Property == 'background'){
                ...
                || value[i].Property == 'font-weight' || value[i].Property == 'font-size' || value[i].Property == 'color'){
                  ...
                  $(`.${value[i].Control}`).find('.textimportInput').css(value[i].Property,value[i].Value) 
                }
                if(value[i].Property == 'StrokeThickness'){
                  $(`.${value[i].Control}`).css('padding',value[i].Value)
                }
          ...
          if(value[i].Property == 'Move'){
            ...
            if(name == 'NumberScreen'){  //数值筛选
                if(value[i].Property == 'background'){
                  $(`.${value[i].Control}`).css('background',value[i].Value)
                  $(`.${value[i].Control}`).find('.elSelect').css('background',value[i].Value)
                  $(`.${value[i].Control} .el-select-dropdown`).css('background',value[i].Value)
                }   
                if(value[i].Property == 'Stroke'){
                  var borderColor = `linear-gradient(to right,${value[i].Value},${value[i].Value}) 1 10`
                  $(`.${value[i].Control}`).find('.elSelect').css('border-image',borderColor)
                  $(`.${value[i].Control}`).find('.elInput1').css('border-image',borderColor)
                  $(`.${value[i].Control}`).find('.elInput2').css('border-image',borderColor)
                }
                if(value[i].Property == 'StrokeThickness'){
                  $(`.${value[i].Control}`).find('.elSelect').css('border-width',value[i].Value)
                  $(`.${value[i].Control}`).find('.elInput1').css('border-width',value[i].Value)
                  $(`.${value[i].Control}`).find('.elInput2').css('border-width',value[i].Value)
                }
                ...
                || value[i].Property == 'font-family' || value[i].Property == 'text-decoration'){
                  ...
                  if(value[i].Property != 'text-decoration'){
                    $(`.${value[i].Control}`).find('input').css(value[i].Property,value[i].Value)
                  }
                  $(`.${value[i].Control}`).find('.elInput1').css(value[i].Property,value[i].Value)
                  $(`.${value[i].Control}`).find('.elInput2').css(value[i].Property,value[i].Value)
                }
                if(value[i].Property == 'opacity'){
                  $(`.${value[i].Control}`).css(value[i].Property,value[i].Value) 
                }
        ...
        if(value[i].Property == 'Move'){
            ...
            if(name == 'mediaplayer'){  //视频控件
              if(value[i].Property == 'opacity'){
                  $(`.${value[i].Control}`).css(value[i].Property,value[i].Value) 
              }
               if(value[i].Property == 'Source'){
                  $(`.${value[i].Control}`).find('video')[0].src =  `/images/${value[i].Value}`
                  setTimeout(()=>{
                    this.$refs.Video.videoFun(value[i].Control,`/images/${value[i].Value}`)
                  },500)
              }
                  console.log($(`.${value[i].Control}`).parents('.divvideo')[0])
                  let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`).parents('.divvideo')[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`).parents('.divvideo')[0].style.width);
                 let moveleft = parseFloat($(`.${value[i].Control}`).parents('.divvideo')[0].style.left);
                 let movetop = parseFloat($(`.${value[i].Control}`).parents('.divvideo')[0].style.top);
                 let moveindex = -1
                 let movetime = 0
                 let movetype = 0
                 let movehtml = ``
         ...
         if(value[i].Property == 'Move'){
                ...
                if(movehtml){
                  ...
                  if(!$(`.${value[i].Control}`).parents('.divvideo').is(':animated')){
                     movehtml=`$('.`+value[i].Control+`').parents('.divvideo')`+movehtml
                     eval(movehtml)
                  }
            ...
            if(name == 'alert'){ //报警控件
              ...
              if(value[i].Property == 'HeaderFontFamily'){
                   $(`.${value[i].Control}AlarmTitle`).css('fontFamily',value[i].Value)
              }
              if(value[i].Property == 'HeaderFontSize'){
                 let flooH = $(`.${value[i].Control}alarm_fool`).height()
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('width','auto')
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('fontSize',`${value[i].Value}px`)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_text').css('fontSize',`${value[i].Value}px`)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('lineHeight',`${value[i].Value}px`)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_text').css('lineHeight',`${value[i].Value}px`)
                $(`.${value[i].Control}AlarmTitle`).parents('.alarm_title').css('height',`${value[i].Value}px`)
                $(`.${value[i].Control}alarm_conter`).css('height',`calc(100% - 28px - ${value[i].Value}px - ${flooH}px)`)
               
              }
              if(value[i].Property == 'HeaderForeground'){
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('color',value[i].Value)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_text').css('color',value[i].Value)
              }
              if(value[i].Property == 'HeaderIsBlod'){
                ...
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('font-weight',weight)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_text').css('font-weight',weight)
              }
              if(value[i].Property == 'HeaderTextDecorations'){
                ...
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_no').css('text-decoration',TextDecorations)
                $(`.${value[i].Control}AlarmTitle`).find('.alarm_title_text').css('text-decoration',TextDecorations)
              }
              if(value[i].Property == 'ItemBackground'){
                  $(`.${value[i].Control}`).find('.alarm_conter').css('background',value[i].Value)
              }
               if(value[i].Property == 'color' || value[i].Property == 'font-size' || 
                  value[i].Property == 'font-family' || value[i].Property == 'font-weight' || value[i].Property == 'text-decoration'){
                    ...
                    if(value[i].Property == 'font-size'){
                      $(`.${value[i].Control}`).find('.alarm_fool_num').css('lineHeight',value[i].Value)
                      $(`.${value[i].Control}`).find('.alarm_fool_num').css('lineHeight',value[i].Value)
                      var headH =$(`.${value[i].Control}AlarmTitle`).height()
                      $(`.${value[i].Control}alarm_conter`).css('height',`calc(100% - 28px - ${headH}px - ${value[i].Value})`)
                    }
                  if(value[i].Property != 'color'){
                      $(`.${value[i].Control}`).find('.alarm_fool_num').css(value[i].Property,value[i].Value)
                  }
                  $(`.${value[i].Control}`).find('.alarm_conter_no').css(value[i].Property,value[i].Value)
                  $(`.${value[i].Control}`).find('.alarm_conter_text').css(value[i].Property,value[i].Value)
                  $(`.${value[i].Control}`).find('.alarm_fool_text').css(value[i].Property,value[i].Value)
               }
               if(value[i].Property == 'background'){
                 $(`.${value[i].Control}`).find('.alarm_box2').css(value[i].Property,value[i].Value)
               }
               if(value[i].Property == 'StrokeThickness'){
                 $(`.${value[i].Control}`).css('padding',value[i].Value)
               }  
                ...
                if(value[i].Property == 'opacity'){
                 ...
                 if(value[i].Property == "linestroke"){
                   for(let i1 =0;i1<$(`.${value[i].Control}`).find('.flowline').length;i1++){
                      $($(`.${value[i].Control}`).find('.flowline')[i1]).css('background',value[i].Value)
                   }
               }
                  let bodytop = parseFloat($('.report')[0].style.height) - parseFloat($(`.${value[i].Control}`)[0].style.height);
                 let bodyleft = parseFloat($('.report')[0].style.width) - parseFloat($(`.${value[i].Control}`)[0].style.width);
     ...
     if(value[i].Property == 'Move'){
            ...
            if(name == 'tank'){
               console.log(value)
               if(value[i].Property == 'background'){
                $(`.${value[i].Control}`).find('.tankfill').css(value[i].Property,value[i].Value)
               }
                if(value[i].Property == 'opacity'){
                 $(`.${value[i].Control}`).css(value[i].Property,value[i].Value)
               }
                 if(value[i].Property == "border-radius"){
                  ...
                  if(value[i].Property == "liquidfill"){
                    $(`.${value[i].Control}`).find('.liquidfill').css('background',value[i].Value)
               }

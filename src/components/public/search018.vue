    ...
    <div class="search-container" :class="{ colordiv: $store.state.color == 'grey' }"
        ...
        <div class="search-left" :style="[{ fontSize: 16 * zoom + 'px' }]">
            <div class="search-item" v-for="(item, index) in searchList" :key="index">
                ...
                <el-select v-if="item.type === 'select'" filterable v-model="searchData[item.model]" class="search-select"
                    :placeholder="lang.SCMSConsoleWebApiMySql_PleChoose" @change="change(item.model, $event)"
                    @focus="getZoom()">
                ...
                <el-date-picker v-if="item.type === 'datetimerange'" v-model="searchData[item.model]" type="datetimerange"
                    ...
                    value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                <el-input v-if="item.type === 'key'" v-model="searchData[item.model]" class="touliao_input"
                    :placeholder="item.placeholder || lang.AlarmRecord_History_Keyword" clearable
                    :style="{ width: 215 * zoom + 'px' }" @keyup.enter.native="touliao"></el-input>
            </div>
            <div class="btn green pointer importtant" @click="touliao" :style="[{ fontSize: 16 * zoom + 'px' },
            { height: 40 * zoom + 'px' },
            { width: 100 * zoom + 'px' },
            { left: 495 * zoom + 'px' },
            { lineHeight: 40 * zoom + 'px' }
            ]" :id='cxid'>投料</div>
            <div class="btn grey pointer importtant" @click="clearFn" :style="[{ fontSize: 16 * zoom + 'px' },
            { height: 40 * zoom + 'px' },
            { width: 100 * zoom + 'px' },
            { left: 605 * zoom + 'px' },
            { lineHeight: 40 * zoom + 'px' }
    ...
    watch: {
        'searchData.boilerGroup': {
            handler: function () {
                this.search()
                this.$nextTick(() => {
                    let arr = document.getElementsByClassName('touliao_input');
                    console.log("arr")
                    if (arr && arr.length) {

                ...
                <div class="setdataleft">
                    ...
                    <div class="rolediscrle">
                        ...
                        <input
                            ...
                            :placeholder="
                                lang.UserManage_UserWindow_PasswordHint
                            "
                            :disabled="IsDomain"
                        />
                    </div>
                    <div class="rolediscrle">
                        <span>{{
                            lang.UserManage_UserWindow_ConfirmPassword
                        }}</span>
                        <input
                            type="password"
                            v-model="rolesecond"
                            :placeholder="
                                lang.UserManage_UserWindow_ConfirmPasswordHint
                            "
                            :disabled="IsDomain"
                        />
                    </div>
    ...
    methods: {
        btnPowerData() {
            ...
            this.buttonarr.forEach(item => {
                ...
                } else if (item.RightName == '添加系统用户按钮') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '添加域用户按钮') {
                    this.tj2id = item.RightID;
                } else if (item.RightName == '编辑按钮') {
                    this.bjid = item.RightID;
                } else if (item.RightName == '删除按钮') {
                    this.scid = item.RightID;
                } else if (item.RightName == '签名验证配置按钮') {
                    this.qmid = item.RightID;
                }
            });
            var userid = '';
            ...
            this.$axios({
                method: 'post',
                url: `/api/UserManage/UserManage_CheckAuthority?argUserID=${userid}&argRightID=${this.tj2id}`
            })
                .then(res => {
                    this.tj2show = res.data.data;
                })
                .catch(err => {
                    console.log('err', err);
                });
            ...
            this.$axios({
                method: 'post',
                url: `/api/UserManage/UserManage_CheckAuthority?argUserID=${userid}&argRightID=${this.qmid}`
            })
                .then(res => {
                    this.qmshow = res.data.data;
                })
                .catch(err => {
                    console.log('err', err);
                });

...
<template>
    <div class="login" v-loading="this.$store.state.isShow" :style="[{'width':'100%'},{'height':'100%'},{zoom:a11}]">
        <div class="login_box">
                  ...
                  <div class="middle_conter">
                   <div class="middle_usrename">
                       <span class="el-icon-user"></span>
                        <input @keyup.enter="loginFun()" class="user" type="username" v-model="userName" :placeholder="localData.Login_UserName"/>
                   </div>
                   <div class="middle_password">
                       <span class="el-icon-lock"></span>
                       <input 
                        class="pad" :type="passwordState.passwordType" 
                        v-model="passWord" :placeholder="localData.Login_Password"
                        @keyup.enter="loginFun()" 
                        @keyup="funcFun($event)" 
                       >
                        <img class="password-eye-icon" 
                            @click="showPassword" 
                            :src="passwordState.icon"
                        />
                   </div>
                   <div class="errText" style="display:none;margin-top:15px;color:red;font-size:14px;"><span class="errText1">{{errText}}</span></div>
                   <div class="DAXX" style="display:none;position:absolute;bottom:-23px;left:0;width:150px;height:30px;background:rgba(157,185,252,0.8);">{{localData.Login_CapitalLockHint}}</div>
                </div>
                </div>
            ...
            <div class="lang" >
                <el-select v-model="langValue" class="login-lang" @focus="getZoom()" @change="changeLang($event)" :placeholder="lang.SCMSConsoleWebApiMySql_PleChoose">
                    <el-option

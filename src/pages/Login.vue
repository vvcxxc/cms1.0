 ...
 * @LastEditTime: 2021-01-26 17:57:37
 -->
<template>
  <div class="login" :style="[{'width':'100%'},{'height':'100%'},{zoom:a11}]">
    <div class="login_box">
      <div class="login_top">
        <i class="bol"></i>
          ...
          <div class="middle_conter">
            ...
            <div class="middle_password">
              ...
              <input
                ...
                type="password"
                v-model="passWord"
                :placeholder="localData.Login_Password"
              >
            </div>
            <div class="errText" style="display:none;margin-top:15px;color:red;font-size:14px;">
              <span class="errText1">{{errText}}</span>
            </div>
            <div
              class="DAXX"
...
export default {
    ...
    created() {
        this.langValue = localStorage.getItem('currentLang')
    ...
    methods: {
        ...
        funcFun(event) {
            ...
            if (strlen) {
                var uniCode = this.passWord.charCodeAt(
                ...
                .then(res => {
                    if (res.data.data.HasConfig == 1) {
                        document.querySelector(
                            '.sycLogo1Img'
                        ).src = `/images/${res.data.data.Config.LoginLeftLogo}`;
                        document.querySelector(
                            '.sycLogo2Img'
                        ).src = `/images/${res.data.data.Config.LoginRightLogo}`;
                    }
                })
                .catch(err => {
                    console.log('err', err);
        ...
        setlang(data) {
            ...
            this.$nextTick(() => {
                ...
                $this.localData.Login_CapitalLockHint =
                    data.Login_CapitalLockHint;
                $this.localData.Login_HintMessage_Empty =
                    data.Login_HintMessage_Empty;
                $this.localData.Login_HintMessage_Error =
                    data.Login_HintMessage_Error;
                $this.localData.Login_HintMessage_MessReStart =
                    data.Login_HintMessage_MessReStart;
                $this.localData.Login_HintMessage_Hint =
                    data.Login_HintMessage_Hint;
            });
        },

        setdropLang(value) {
            if (value === 'Main_Language_EN') {
                this.languages = [
                    {
                        value: 'Main_Language_ZH',
                        label: 'Chinese'
                    },
                    {
                        value: 'Main_Language_EN',
                        label: 'English'
                    }
                ];
            } else if (value === 'Main_Language_ZH') {
                this.languages = [
                    {
                        value: 'Main_Language_ZH',
                        label: '中文'
                    ...
                    if (res.data.data.LoginIsSuccess) {
                        ...
                        this.$axios({
                            ...
                            url:
                                ...
                                res.data.data.SCMSUserName +
                                '&Type=Main_Record_WebLogin',
                        ...
                        this.$axios({
                            method: 'post',
                            url: `/api/LoginABManage/LoginABManagement?argUserAccount=${res.data.data.SCMSUserName}`
                        }).then(resp => {
                            if (resp.data.code == 0) {
                                console.log(" resp.data.data.Permit", resp.data.data.Permit ,typeof(resp.data.data.Permit))
                                sessionStorage.setItem(
                                    'isBSystem',
                                    resp.data.data.Permit
                                );
                            }
                        });
                    } else {

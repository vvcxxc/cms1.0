...
export function generateArray(table) {
  ...
  var ranges = []
 console.log("data",Date.parse(new Date()),new Date())
    ...
    for (var C = 0; C < columns.length; ++C) {
      ...
      if(cell.style.backgroundColor){
        ...
        if (cell.style.backgroundColor.includes('rgba')) {
          style.fill.fgColor = {
            rgba: cell.style.backgroundColor
          }
        } else {
          style.fill.fgColor = {
        ...
        if($(cell).hasClass('item')){
          style.fill.bgColor =  { indexed:64 }
          style.fill.fgColor = {rgb:colorRGBtoHex("(249,222,224)")}
          }else{
            style.fill.bgColor =  { indexed:64 }
     ...
     if($(cell).css('border-right-style')!=='none'&&$(cell).css('border-right-style')){
      ...
      if($(cell).css('border-bottom-style')!=='none'&&$(cell).css('border-bottom-style')){
        style.border.bottom = {
          style:'thin',
          color:{
      ...
      if (rowspan || colspan) {
        ...
        colspan = colspan || 1
        // console.log($(cell))
        // console.log(cell.getAttribute("colspan"))
        ranges.push({
          s: {
            r: R,
            c: outRow.length
...
export function export_table_to_excel(theTable,width,wsrows) {
...
//  console.log(ws)
  ...
  // }
  console.log("ws",ws)
  Object.keys(wb.Sheets.SheetJS).forEach(function(key){
    if(key !== "!ref"&&key!=='!cols'&&key!=='!merges'&&key!=='!rows'){
      if(wb.Sheets.SheetJS[key].v==""&&JSON.stringify(wb.Sheets.SheetJS[key].s.border)=='{}'&&wb.Sheets.SheetJS[key].s.fill.fgColor.rgb=="FFffffff"){
        delete wb.Sheets.SheetJS[key]
      }
    }
  })
  console.log("wb",wb)
  ...
  console.log("data1",Date.parse(new Date()),new Date())
  saveAs(
    new Blob([s2ab(wbout)], {

<template>
    <div style="height: 100%">
        <div class="management">
            <div class="management-header">
                <span class="title" :style="{ fontSize: 20 * a11 + 'px' }">物料BOM</span>
                <div class="management-btn">
                    ...
                    <el-upload action="#" accept=".xlsx" :show-file-list="false" :http-request="uploadExcel">
                        <div class="import" @click="handleUpload">导入</div>
                    </el-upload>
                    <div class="import" @click="exportFn">导出</div>
                </div>
            </div>
            <el-table :data="tableData" style="width: 100%; overflow: auto" stripe @row-click="handleRowChange"
                :height="`calc(100% - ` + a11 * 28 + `px)`" :style="{
                    fontSize: a11 * 15 + 'px',
                    width: a11 * 1690 + 'px',
    ...
    padding: '0',
        ...
        <CodeDialog v-if="dialogVisible" :appendToBody="true" :visible="dialogVisible" :title="dialogTitle"
            ref="managementCodeDialogRef" :dialogFormData="dialogFormData" @close="dialogVisible = false"
            @confirm="onDialogConfirm" :currentRowData="broCompData.currentRowData" />
    </div>
</template>
...
export default {
    ...
    computed: {
        dialogTitle: function () {
            const TITLE_ENUM = {
                [BTN_EMIT.ADD]: '添加物料BOM',
                [BTN_EMIT.EDIT]: '编辑物料BOM',
            };
    ...
    data() {
        return {
            ...
            managementHeader: [
                ...
                { prop: 'Unit', label: '单位' },
                { prop: 'Usage', label: '物料用量' },
                { prop: 'WorkName', label: '工序' },
                { prop: 'Sequence', label: '扫码顺序标识' },
                { prop: 'WriteVariables', label: '写入变量' },
    ...
    methods: {
        getPower() {
            ...
            this.buttonarr.forEach(item => {
                if (item.RightName == '物料BOM管理-添加按钮') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '物料BOM管理-编辑按钮') {
                    this.bjid = item.RightID;
                } else if (item.RightName == '物料BOM管理-删除按钮') {
                    this.scid = item.RightID;
                } else if (item.RightName == '物料BOM管理-导入按钮') {
                    this.drid = item.RightID;
                } else if (item.RightName == '物料BOM管理-导出按钮') {
        ...
        uploadExcel(file) {
            ...
            this.$axios({
                method: 'post',
                url: `/api/MaterialBOMManage/ImportExcel?LoginUserName=${_name}`,
                data: formData
            }).then((result) => {
        ...
        exportFn() {
            ...
            window.open(`/api/MaterialBOMManage/ExportBomExcel?ProductTypeId=${this.broCompData.currentRowData.ID}`)
        },
        handleRowChange(row) {
            this.tableSelect = row;
        },
        async initData(val) {
            const data = await api.getAsynsBomt(val);
        ...
        async onBtnClick(item) {
            ...
            if (BTN_EMIT.EDIT === keyword) {
                ...
                const candel = await api.VerifyBOM(this.tableSelect.ID, 'edit');
                const candelRes = _.get(candel, 'data.data', []);
                if (candelRes != "OK") {
                    this.tipText = _.get(candel, 'data.msg');
                    this.noCancel = true
                    this.isPopShow = true;
                    return
                }
                ...
                const candel = await api.VerifyBOM(this.tableSelect.ID, 'del');
                const candelRes = _.get(candel, 'data.data', []);
                if (candelRes != "OK") {
                    this.tipText = _.get(candel, 'data.msg');
                    this.noCancel = true
                ...
                this.changeTipPopConfig(`是否确认删除物料[${this.tableSelect.MaterialName}]`, false)
                return;
        ...
        async tipCallBack(str) {
            ...
            this.noCancel = true;
            if (str == 'yes') {
                const result = await api.DelBom(this.tableSelect.ID);
                // const resultStatus = result.data.code === 0 ? 'success' : 'warning'
                const message = result.data.code === 0 ? '删除成功' : _.get(result, 'data.msg')
                // this.$message({
                //     message: message,
                //     type: resultStatus,
                // });
        ...
        async onDialogConfirm() {
            ...
            formData = {
                ...formData,
                ProductTypeID: this.broCompData.currentRowData.ID,
            };
            ...
            if (BTN_EMIT.ADD === this.currentBtnEmitType) {
                interResult = await api.AddOrEditBom(formData, _name);

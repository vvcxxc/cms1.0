...
export default {
    ...
    methods: {
        ...
        async uploadExcel(file) {
            ...
            try {
                result = await this.$confirm(
                    '是否覆盖已关联BOM产品型号数据，如覆盖，则已存在的记录将被导入的数据所覆盖?',
                    '提示',
                    {
                        confirmButtonText: '是',
                        cancelButtonText: '否',
            ...
            const isCover = result == 'confirm' ? true : false;
            console.log(result, 'result >>>', isCover);
            ...
            this.$axios({
                method: 'post',
                url: `/api/Bom/ImportBom?productTypeID=${this.currentMid}&LoginUserName=${_name}&isCover=${isCover}`,
                data: formData,
            }).then((result) => {
        ...
        exportFn() {
            ...
            window.open(
                `http://${apiHref}/api/Bom/ExportBom?ProductTypeID=${this.currentMid}`
            );
        },
        handleRowChange(row) {
        ...
        async onBtnClick(item) {
            ...
            if (BTN_EMIT.EDIT === keyword) {
                ...
                this.changeTipPopConfig(
                    `是否确认删除物料[${this.tableSelect.MaterialName}]`,
                    false
                );
                return;
        ...
        async onDialogConfirm() {
            ...
            if (BTN_EMIT.ADD === this.currentBtnEmitType) {
                interResult = await api.addBom(formData);
            } else if (BTN_EMIT.EDIT === this.currentBtnEmitType) {
                interResult = await api.updateBom(
                    formData,
                    this.tableSelect.Id
                );
            }

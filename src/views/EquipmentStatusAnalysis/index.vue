            ...
            <div class="search-content">
                <div class="search-left">
                    ...
                    <el-select :style="{ width: '150px' }" v-model="Equipment"
                        ...
                        <el-option v-for="item in EquipmentList" :key="item.AID" :label="item.Name" :value="item.AID">
                        </el-option>
                    </el-select>
                    <div>班次</div>
            ...
            <div class="alarm-page" ref="alarmPage">
                ...
                <div class="table-box">
                    <el-table highlight-current-row :cell-style="{ borderColor: '#E4E7ED' }"
                        ...
                        <el-table-column prop="SummaryDate" label="日期" :show-overflow-tooltip="true"></el-table-column>
                        ...
                        <el-table-column v-for="(item, idx) in LineDurations" :prop="item.Name" :label="item.Name"
                            :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                                {{ scope.row[`DisplayMult${Number(idx) + 1}Duration`] }}
                            </template>
                        </el-table-column>
    ...
    methods: {
        ...
        exportFn() {
            ...
            html2canvas(canvasID).then(canvas => {
                ...
                a.setAttribute('download', '设备状态分析.jpg');
        ...
        getData() {
            this.$axios({
                ...
                if (res.data.code == 0) {
                    ...
                    this.LineDurations = res.data.data.LineDurations || []
        ...
        drawLine() {
            ...
            let option = {
                ...
                color: this.LineDurations.map((item) => item.Color),
                legend: {
                    data: this.LineDurations.map((item) => item.Name),
                },
                calculable: true,
                xAxis: [
                    {
                ...
                series: this.LineDurations.map((item) => ({
                    name: item.Name,
                    type: 'bar',
                    barWidth: 30,  // 根据实际情况设置合适的值
                    data: item.Data,
                    lineStyle: {
                        normal: {
                            color: item.Color,

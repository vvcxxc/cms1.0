                                ...
                                <el-table-column prop="ContentGroup" label="分组" :show-overflow-tooltip="true">
                                    <template slot-scope="scope">
                                        <el-select placeholder="请选择" v-model="scope.row.ContentGroup">
                                            <el-option v-for="item in workStepList" :key="item.SID" :label="item.StepName"
                                                :value="item.StepName">
                                            </el-option>
                                        </el-select>
    ...
    methods: {
        ...
        handleClick(str) {
			...
			if (!JSON.parse(sessionStorage.getItem('userInfo1'))) {
                ...
                this.$axios({
                    method: 'post',
                    url: `api/FormulaManage/FormulaManage_UpdateFormulaTemplate?loginUserName=${userid}`,
                    data: this.tempList.map(_ => ({
                        ..._,
                        Projects: _.Projects.map(_2 => ({
                            ..._2,
                            ContentGroupId: this.workStepList.find(_3 => _3.StepName == _2.ContentGroup) ? this.workStepList.find(_3 => _3.StepName == _2.ContentGroup).SID : ''
                        }))
                    }))
                })
                    .then(res => {
        ...
        getProcess() {
            this.$axios({
                url: `/api/ProcessParameterConfigure/QueryworkStepSelect`,
                method: 'post',
            }).then((res) => {
                if (res.data.code == 0) {

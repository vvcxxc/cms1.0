        ...
        <div class="myDialog" id="myDialog">
            ...
            <div class="popContent">
                <el-table ref="projTable" highlight-current-row :cell-style="{ borderColor: '#E4E7ED' }"
                    ...
                    <el-table-column prop="PartCode" label="定子总成零件号(8位)" :show-overflow-tooltip="true" width="180px">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.PartCode" />
                        </template>
                    </el-table-column>
                    <el-table-column prop="AssembleCode" label="生产装配代码(14位)" :show-overflow-tooltip="true" width="180px">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.AssembleCode" />
                        </template>
                    </el-table-column>
                    <el-table-column prop="SupplierCode" label="供应商代码(9位)" :show-overflow-tooltip="true" width="150px">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.SupplierCode" />
                        </template>
                    </el-table-column>
                    <el-table-column prop="SpecialCode" label="特殊代码" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.SpecialCode" />
                        </template>
                    </el-table-column>
                    <el-table-column prop="DeliveryStatus" label="交货状态" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.DeliveryStatus" />
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods: {
        ...
        tipCallBack(str) {
            if (str == 'yes') {
                const target = this.dataList[this.curIdx]
                ...
                if (target.Id !== DEFAULT_VALUE) {
                    this.DelProductTypes.push(target.Id)
                }
            }
            this.noCancel = true;
        ...
        handleClick(str) {
            if (str == 'yes') {
                this.$axios({
                    ...
                    data: {
                        ProjectProductTypes: this.dataList,
                        DelProductTypes: this.DelProductTypes,   
                    }
                })
                    .then(res => {
                        if (res.data.code == 0) {

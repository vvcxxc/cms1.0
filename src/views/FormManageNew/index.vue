    ...
    <div class="FormManageNew-container">
        <div class="FormManageNew-flex">
                ...
                <div class="item-search">
                    <div class="search-left">
                        <div
                            ...
                            @click="openEditFormua('添加配方')"
                        >
                            添加
                        </div>
                        <div class="search-btn" @click="openEditTemplate">
                            模板管理
                        </div>
                    ...
                    <div class="search-right">
                        <div class="search-btn type" @click="openEditType">
                            型号管理
                        </div>
                    </div>
                </div>
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope">
                                <div class="green" v-if="scope.row.IsApp">
                                    应用中
                                </div>
                                <div v-else>未下发</div>
                            </template>
                        ...
                        >
                            <template slot-scope="scope">
                                ...
                                <div
                                    class="table-action"
                                    @click="
                                        openEditFormua('修改配方', scope.row)
                                    "
                                >
                                    <img
                                        src="../../assets/images/icon_pensoil.png"
                                        alt
                                    />
                                </div>
                                <div
            ...
            <div class="FormManageNew-item">
                ...
                <div class="item-search">
                    <div class="search-left">
                        <div class="search-btn log" @click="openLog">
                            监控日志
                        </div>
                        <div class="search-btn" @click="refreshFn">
                            刷新实时值
                        </div>
                        <div class="search-btn" @click="coverFn">
                            覆盖配方值
                        </div>
                        <div class="search-btn log" @click="checkDrPower">
                            <el-upload
                                action="#"
                                accept=".xlsx,.xls"
                        ...
                        <div class="search-btn sop" @click="onSop">SOP管理</div>
                    </div>
                    <div class="search-right">
                ...
                <div class="item-table">
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope">
                                <div class="green" v-if="scope.row.CompareRes">
                                    {{ scope.row.RealValue }}
                                </div>
        ...
        <EditFormua
            v-if="EditFormuaShow"
            :title="EditFormuaTitle"
            :selectItem="EditFormuaRow"
            @callback="EditFormuaCallBack"
        ></EditFormua>
        <EditGroup
            v-if="EditGroupShow"
            :title="EditGroupTitle"
            :selectItem="EditGroupRow"
            @callback="openEditGroupCallBack"
        ></EditGroup>

        <EditType v-if="EditTypeShow" @callback="openEditTypeCallBack">
        </EditType>
        <EditTemplate
            v-if="EditTemplateShow"
            @callback="openEditTemplateCallBack"
        ></EditTemplate>
        <Log v-if="logShow" @callback="logCallBack"></Log>
        <SopDialog
            v-if="sopShow"
            :dialogConfig="sopConfig"
...
import SopDialog from './component/sopDialog.vue';
...
export default {
    ...
    data() {
        return {
            ...
            xfsbzid: '',
            xfsbzshow: true,
            authBtnSopManagementID: '',
            authBtnSopManagementStatus: true,
        };
    },
    created() {
        this.getData();
    ...
    computed: {
        rightList() {
            if (
                !this.curLeft ||
                !this.curLeft.TemplateProjects ||
                !this.curLeft.TemplateProjects.length
            ) {
                return [];
            }
    ...
    methods: {
        ...
        getPower() {
            ...
            this.buttonarr.forEach((item) => {
                ...
                } else if (item.RightName === 'SOP管理按钮') {
                    this.authBtnSopManagementID = item.RightID;
                }
            });
            ...
            this.$axios({
                method: 'post',
                url: `/api/UserManage/UserManage_CheckAuthority?argUserID=${userid}&argRightID=${this.authBtnSopManagementID}`,
            })
                .then((res) => {
                    this.authBtnSopManagementStatus = res.data.data;
                })
                .catch((err) => {
                    console.log('err', err);
                });
        ...
        onSop() {
            if (!this.authBtnSopManagementStatus) {
                this.tipText = this.lang.NoOperationAuthority;
                this.isTipShow = true;
                return;
            }
            this.sopConfig = this.curLeft;
            this.sopShow = true;
        },
        sopCallBack() {
            this.sopShow = false;
        ...
        tipCallBack(str) {
            ...
            if (str == 'yes') {
                this.$axios({
                    method: 'post',
                    url: `/api/FormulaManage/FormulaManage_DelFormula?argFId=${this.curLeft.FID}`,
                }).then((res) => {
                    if (res.data.code == 0) {
                        this.isTipShow = true;
                        this.tipText = '删除成功！';
        ...
        tableRowClick(row) {
            ...
            if (
                ...
                this.curLeft.Projects.length
            ) {
                this.$nextTick(() => {
                    this.$refs.projTable.setCurrentRow(
                        this.curLeft.Projects[0]
                    );
                });
            }
        },
            ...
            this.$axios({
                ...
                if (res.data.code == 0) {
                    ...
                    if (this.dataList1.length) {
                        if (noRestIdx) {
                            let _obj = this.dataList1.find(
                                (item) => item.FID == this.curLeft.FID
                            );
        ...
        async uploadExcel(file) {
            ...
            this.$axios({
                method: 'post',
                url: `/api/FormulaManage/FormulaManage_ImportFormula?LoginUserName=${_name}`,
                data: formData,
            }).then((result) => {
                const message =
                    result.data.code === 0
                        ? _.get(result, 'data.data') || '导入成功'
                        : _.get(result, 'data.msg') || '导入失败';
        ...
        readExcel(e) {
            ...
            fileReader.onload = (ev) => {
                ...
                for (a in ws) {
                    ...
                    ws[a].Number =
                        ws[a][this.lang.FormulaManage_FormulaSetting_Number];
                    ws[a].FormulaDescription =
                        ws[a][
            ...
            this.$axios({
                ...
                if (res2.data.code == 0) {
                    this.curLeft.TemplateProjects.forEach((item) => {
                        if (
                            res2.data.data.some(
                                (item2) => item.PID == item2.PID
                            )
                        ) {
                            let _obj = res2.data.data.find(
                                (item2) => item.PID == item2.PID
                            );
        ...
        coverFn() {
            ...
            this.$axios({
                method: 'post',
                url: `/api/FormulaManage/ModifyFormula?tid=${this.curLeft.TID}&fid=${this.curLeft.FID}&update=1`,
            }).then((res) => {
                if (res.data.code == 0) {
                    this.tipText = '覆盖成功！';
                    this.isTipShow = true;
                    // this.getData(true);
                    const index = this.dataList1.findIndex(
                        (item) => item.FID == this.curLeft.FID
                    );
                    // this.dataList1[index] = res.data.data;
                    this.$set(this.dataList1, index, res.data.data);
                    this.$nextTick(() => {
                        this.$refs.formualTable.setCurrentRow(
                            this.dataList1[index]
                        );
                        this.curLeft = this.dataList1[index];
                    });
                    console.log({ index }, this.dataList1);
                } else {
                    this.tipText = res.data.msg;
                    this.isTipShow = true;
                }
        ...
        saveFn() {
            ...
            this.$axios({
                ...
                data: { ...this.curLeft, Items: this.curLeft.TemplateProjects },
            }).then((res) => {
                if (res.data.code == 0) {
                    this.tipText = '保存成功！';
                    this.isTipShow = true;

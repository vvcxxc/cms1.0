			...
			<div class="FormManageNew-item">
				...
				<div class="item-search">
					<div class="search-left">
						...
						<div class="search-btn log" @click="checkDrPower">导入
							<a href="javascript:;" class="file">
								<input type="file" ref="upload" @change="readExcel" class="outputlist_upload"
									accept=".xls, .xlsx" />
							</a>
				...
				<div class="item-table">
					<el-table @row-click="tableRowClick2" highlight-current-row :cell-style="{ borderColor: '#E4E7ED' }"
						...
						<el-table-column prop="ParameterTypeName" label="参数类型" :show-overflow-tooltip="true"></el-table-column>
...
export default {
	...
	methods: {
		...
		coverFn() {
			...
			this.$axios({
				method: 'post',
				url: `/api/FormulaManage/ModifyFormula?tid=${this.curLeft.TID}&fid=${this.curLeft.FID}&update=0`,
			}).then(res => {
				if (res.data.code == 0) {
					this.tipText = '覆盖成功！'
					this.isTipShow = true;

    ...
    <div class="MainPage-container-page">
        ...
        >
            ...
            <div class="MainPage-banner">
                <div
                    class="MainPage-banner-mask"
                    v-show="this.showListType == 'Online'"
                >
                    <div
                        class="MainPage-banner-info green"
                        v-for="(item, idx) in this.OnlineList"
                        :key="idx"
                    >
                        {{ item }}
                    </div>
                ...
                <div
                    class="MainPage-banner-mask"
                    v-show="this.showListType == 'OffLine'"
                >
                    <div
                        class="MainPage-banner-info"
                        v-for="(item, idx) in this.OffLineList"
                        :key="idx"
                    >
                        {{ item }}
                    </div>
                ...
                <div
                    class="MainPage-banner-mask"
                    v-show="this.showListType == 'Normal'"
                >
                    <div
                        class="MainPage-banner-info green"
                        v-for="(item, idx) in this.NormalList"
                        :key="idx"
                    >
                        {{ item }}
                    </div>
                ...
                <div
                    class="MainPage-banner-mask"
                    v-show="this.showListType == 'Alarm'"
                >
                    <div
                        class="MainPage-banner-info red"
                        v-for="(item, idx) in this.AlarmList"
                        :key="idx"
                    >
                        {{ item }}
                    </div>
            ...
            <div class="MainPage-container">
                <div class="MainPage-num">
                    <div
                        class="MainPage-num-item"
                        @click="PLCOnlineSentInfoToMain"
                    >
                    ...
                    <div
                        class="MainPage-num-item"
                        @click="PLCOfflineSentInfoToMain"
                    >
                ...
                <div class="MainPage-label"  v-if="!isBSystem">
                    <div class="MainPage-label-title">产量统计</div>
                    <div class="MainPage-label-unit">钵</div>
                </div>

                <div class="Output-statistics"  v-if="!isBSystem">
...
export default {
    ...
    methods: {
        ...
        PLCStateSentCountToMain() {
            this.$axios
                .post(`/api/MainWindowData/PLCStateSentCountToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
                        let { Alarm, Normal, OffLine, Online } = res.data.data;
                        this.Alarm = Alarm;
                        this.Normal = Normal;
        ...
        PLCOnlineSentInfoToMain() {
            this.$axios
                .post(`/api/MainWindowData/PLCOnlineSentInfoToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
        ...
        PLCOfflineSentInfoToMain() {
            this.$axios
                .post(`/api/MainWindowData/PLCOfflineSentInfoToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
        ...
        PLCNormalSentInfoToMain() {
            this.$axios
                .post(`/api/MainWindowData/PLCNormalSentInfoToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
        ...
        PLCAlarmSentInfoToMain() {
            this.$axios
                .post(`/api/MainWindowData/PLCAlarmSentInfoToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
        ...
        OutputStatisticsSendToMain() {
            this.$axios
                .post(`/api/MainWindowData/OutputStatisticsSendToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
                        let {
                            ProductionOfShiftA,
                        ...
                        this.ProductionOfShiftA = ProductionOfShiftA;
                        this.ProductionOfShifB = ProductionOfShifB;
                        this.ProductionOfYesterday = ProductionOfYesterday;
        ...
        PowerCalculateSendToMain() {
            this.$axios
                .post(`/api/MainWindowData/PowerCalculateSendToMain`)
                .then((res) => {
                    if (res.data.code == 0) {
                        let { TenMinutesPower, YesterdayPower, MonthPower } =
                            res.data.data;
                        this.TenMinutesPower = TenMinutesPower;
        ...
        FuelCalculateSendToMain() {
            this.$axios
                .post(
                    `/api/MainWindowData/FuelCalculateSendToMain?KilnID=${Number(
                ...
                .then((res) => {
                    if (res.data.code == 0) {
                        ...
                        for (let i in DataList) {
                            NowTime[DataList[i].Tag] = DataList[i].NowTime;
                            LastMonth[DataList[i].Tag] = DataList[i].LastMonth;
                            CurMonth[DataList[i].Tag] = DataList[i].CurMonth;
                        }
                        let arr = [NowTime, LastMonth, CurMonth];
                        this.tableTitleList = DataList.map((item) => {
                            return item.Tag;
                        });
                        this.tableData = arr;
                    } else {
    ...
    watch: {
        // '$store.state.PLCCount': {
        //     //plc信息
        //     deep: true,
        //     handler: function (n, o) {
        //         let { Alarm, Normal, OffLine, Online } =
        //             this.$store.state.PLCCount;
        //         if (
        //             !isNaN(Alarm) &&
        //             !isNaN(Normal) &&
        //             !isNaN(OffLine) &&

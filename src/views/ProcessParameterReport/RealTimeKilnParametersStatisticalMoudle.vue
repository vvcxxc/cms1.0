<template>
    <div class="ProcessParameterReport-Page" :class="{ blackBlueBg: $store.state.color === 'blackBlue' }">
        ...
        <div class="page-center">
            <div class="select">
                <div class="page-title">窑炉参数表（实时）</div>
                <div class="select-l">
                    <div class="select-text" v-if="!isBSystem">查询时间：</div>
                    <el-date-picker v-if="!isBSystem" :key="$store.state.color === 'blackBlue' ? 'blackBlueBg' : 'normal'"
                        :popper-class="$store.state.color === 'blackBlue' ? 'blackBlueBg' : 'normal'" type="datetime"
                        v-model="seleTime" placeholder="查询时间" default-time="12:00:00"
                        value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    <el-button class="select-btn" @click="GetProcessParamDataByTime">{{ isBSystem ? '刷新' : '查询'
                    }}</el-button>
                </div>
                    ...
                    <el-table :data="tableData" :span-method="objectSpanMethod" :border="true" style="width: 100%"
                        ...
                        <el-table-column prop="LineIndex" label="碳化线编号" width="100" :resizable="false" align="center">
                            <template slot-scope="scope">
                                <div v-if="scope.row.LineIndex.indexOf('记录人') >= 0" class="left white">{{
                                    scope.row.LineIndex }}</div>
                                <span v-else class="white">{{ scope.row.LineIndex }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="LineName" width="90" align="center">
                            <template slot-scope="scope">
                                <div v-if="scope.row.LineName.indexOf('确认人') >= 0" class="left white">{{ scope.row.LineName
                                }}</div>
                        ...
                        <el-table-column prop="FurnacePressure" width="100" label="炉压（pa）" align="center">
                            <template slot-scope="scope">
                                <input type="text" v-if="isBSystem" class="rowInput" v-model="scope.row.FurnacePressure"
                                    @blur="isNumber">
                                <span v-else>{{ scope.row.FurnacePressure }}</span>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
    ...
    methods: {
        exportTable() {
            ...
            require.ensure([], () => {
                ...
                this.json2excel(excelDatas, '窑炉参数表（实时）', true, 'xlsx');
            });
        },
        ...
        GetProcessParamDataByTime() {
            ...
            this.$axios
                .get(
                    `/api/ProcessParameters/GetProcessParamDataByTime?time=${this.seleTime}&isBSystem=${this.isBSystem}`
                )
                .then(res => {
                    if (res.data.code == 0) {
        ...
        saveTable() {
            ...
            this.$axios
                .post(
                    `/api/ProcessParameters/EditProductionParamData?time=${this.seleTime}`,
                    arr
                )
                .then(res => {
                    //弹窗保存成功

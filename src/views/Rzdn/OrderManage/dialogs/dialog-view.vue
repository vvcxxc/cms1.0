  ...
  >
    <div class="container" >
      <div class="header">
        ...
        <el-button class="common_button delBtn" @click="delOrder">删除</el-button>
      </div>
      <div class="main" >
        <Table
          isSelection
...
import { GetWorkStationRecords, DeleteWorkStationRecords, Detail } from '@/api/rzdn/orderManage.js'
...
export default {
  ...
  methods: {
    ...
    delOrder() {
      ...
      let $this = this
      let ids = this.multipleSelection.map(item => item.WorkStationRecordId)
      let titles = this.multipleSelection.map(item => item.ProductId)
      let titleStr = titles.join(',<br>')

      this.confirm_Pop(this, `是否删除产品：<br>${titleStr}？`, function() {
        DeleteWorkStationRecords({WorkStationIds: ids}).then(res => {
          if (res.data.IsSuccess) {

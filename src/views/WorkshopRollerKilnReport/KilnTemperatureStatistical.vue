        ...
        <div class="page-center">
            <div class="select">
                <div class="select-l">
                    ...
                    <el-date-picker :popper-class="$store.state.color === 'blackBlue' ? 'blackBlueBg' : 'normal'"
                        type="date" v-model="selectTime" placeholder="查询时间" default-time="12:00:00"
                        value-format="yyyy-MM-dd"></el-date-picker>
                    <div class="select-text2" v-if="isBSystem">-</div>
                    <el-date-picker v-if="isBSystem"
                        :popper-class="$store.state.color === 'blackBlue' ? 'blackBlueBg' : 'normal'" type="date"
                        v-model="selectTime2" placeholder="结束时间" default-time="12:00:00"
                        value-format="yyyy-MM-dd"></el-date-picker>
            ...
            <div class="content">
                <div class="table-content">
                    <div class="table-title">
                        ...
                        <div class="table-title">窑炉温度统计表</div>
                    </div>
                    <div v-if="!isBSystem">
                        <div class="table-item" v-for="item in tableList" :key="item.Time">
                            ...
                            <div class="table-info">
                                <div class="table-info-child">
                                    <div class="clomn">
                                        ...
                                        <div class="row">输出功率</div>
                                        <div class="row">输出电流</div>
                                        <div class="row">输出电压</div>
                                    </div>
                                    <div class="clomn"
                                        v-for="_ in item.WorkshopRecordInfoList.filter((i) => { return i.Tag.indexOf('T') >= 0 || i.Tag == '处理炉' })"
                                        :key="_.Tag">
                                        <div class="row">{{ _.Tag }}</div>
                                        <div class="row">{{ _.SetTemperature }}</div>
                                        <div class="row">{{ _.CurTemperature }}</div>
                                        <div class="row">{{ _.OutPower }}</div>
                                        <div class="row">{{ _.OutElectricity }}</div>
                                        <div class="row">{{ _.OutVoltage }}</div>
                                    </div>
                                </div>
                                <div class="table-info-child">
                                    <div class="clomn">
                                        ...
                                        <div class="row">输出电流</div>
                                        <div class="row">输出电压</div>
                                        <div class="row">氮气流量</div>
                                    </div>
                                    <div class="clomn"
                                        v-for="_ in item.WorkshopRecordInfoList.filter((i) => { return i.Tag.indexOf('U') >= 0 || i.Tag == '炉压' })"
                                        :key="_.Tag">
                                        <div class="row">{{ _.Tag }}</div>
                                        <div class="row">{{ _.SetTemperature }}</div>
                                        <div class="row">{{ _.CurTemperature }}</div>
                                        ...
                                        <div class="row">{{ _.OutElectricity }}</div>
                                        <div class="row">{{ _.OutVoltage }}</div>
                                        <div class="row">{{ _.NitrogenFlow }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="table-info">
                                <div class="table-info-child">
                                    ...
                                    <div class="clomn"
                                        ...
                                        <div class="row">{{ _.SetTemperature }}</div>
                                        <div class="row">{{ _.CurTemperature }}</div>
                                        <div class="row">{{ _.TargetTemperature }}</div>
                                    </div>
                                </div>
                                <div class="table-info-child">
                                    <div class="clomn">
...
export default {
    ...
    methods: {
        ...
        WorkshopRecordSentToPage() {
            ...
            let isBSystem = eval(sessionStorage.getItem('isBSystem')) || false;
            let url = isBSystem ? `/api/WorkshopRecord/WorkshopRecordSentToPage?KilnID=${this.curSelectItem}&st=${this.selectTime}&et=${this.selectTime2}` : `/api/WorkshopRecord/WorkshopRecordSentToPage?KilnID=${this.curSelectItem}&time=${this.selectTime}`
            this.$axios
                .get(
                    url
                )
            ...
            this.$axios
                ...
                .then(res => {
                    if (res.data.code == 0) {
                        ...
                        let temp = []
                        let title = data[0].WorkshopRecordInfoList.map(_ => _.Tag)
                        title.unshift('')
                        title.unshift('时间')
                        if (this.isBSystem) {
                            data.map(_ => {
                                let t1 = { '时间': _.Time, '': '设定温度' },
                                    t2 = { '时间': _.Time, '': '实际温度' },
                                    t3 = { '时间': _.Time, '': '目标温度' };

                                _.WorkshopRecordInfoList.map(_2 => {
                                    t1[_2.Tag] = _2.SetTemperature
                                    t2[_2.Tag] = _2.CurTemperature
                                    t3[_2.Tag] = _2.TargetTemperature
                                })
                                temp.push(t1)
                                temp.push(t2)
                                temp.push(t3)
                            ...
                            data.map(_ => {
                                ...
                                _.WorkshopRecordInfoList.map(_2 => {
                                    t1[_2.Tag] = _2.SetTemperature
                                    t2[_2.Tag] = _2.CurTemperature
                                    t3[_2.Tag] = _2.OutPower
                                    t4[_2.Tag] = _2.OutElectricity
                                    t5[_2.Tag] = _2.OutVoltage
                                    t6[_2.Tag] = _2.NitrogenFlow
                                })
                                temp.push(t1)
                                temp.push(t2)
                                temp.push(t3)
    ...
    mounted() {
        ...
        this.selectTime2 = this.getNowTime();

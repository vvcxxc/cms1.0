       ...
       //所有值
        ...
        timeInit(arr){
            for(let i=0;i<this.dataValue.length;i++){
                ...
                if(index != -1){
                    this.TimePickerListFun(this.data.Data.AssociateDateTimePickerList,this.dataValue[i].class)
        ...
        TimePickerListFun(AssociateDateTimePickerList,className){
             for(let f=0;f<AssociateDateTimePickerList.length;f++){
                  if(AssociateDateTimePickerList[f].ElementName == className){
                      let time1
                      if(AssociateDateTimePickerList[f].InitialTime == '当前时刻'){
                          time1 = moment().format("YYYY-MM-DD HH:mm:ss");
                          this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前小时'){
                          time1 = moment().format("YYYY-MM-DD HH:00:00");
                          this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前日期'){
                           time1 = moment().format("YYYY-MM-DD 00:00:00");
                           this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前周'){
                            var newDa = moment().format("YYYY-MM-DD 00:00:00");
                            var date = moment(newDa);
                            var dow = date.day();
                            time1 = date.subtract(dow-1, 'days').format('YYYY-MM-DD')//本周一
                            time1 = time1 + ' 00:00:00'
                            this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前月份'){
                           time1 = moment().format("YYYY-MM-01 00:00:00");
                           this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前季度'){
                          let currentQuarter = moment().quarter() // 当前是第几季度
                          let currentYear = moment().year()
                          time1 =  moment().startOf('quarter').format("YYYY-MM-DD 00:00:00")
                          this.BeforeTimeFun(time1,AssociateDateTimePickerList[f])
                      }else if(AssociateDateTimePickerList[f].InitialTime == '当前年份'){
        ...
        BeforeTimeFun(time1,AssociateDateTimePickerList){
           let BeforeTimeYear = Number(AssociateDateTimePickerList.BeforeTimeYear)
           let BeforeTimeMonth =  Number(AssociateDateTimePickerList.BeforeTimeMonth)
           let BeforeTimeDay = Number(AssociateDateTimePickerList.BeforeTimeDay)
           let BeforeTimeHour = Number(AssociateDateTimePickerList.BeforeTimeHour)
           let BeforeTimeMinute = Number(AssociateDateTimePickerList.BeforeTimeMinute)
           let BeforeTimeSecond = Number(AssociateDateTimePickerList.BeforeTimeSecond)
           this.resultTime =  moment(time1).subtract(BeforeTimeSecond,"seconds").subtract(BeforeTimeMinute, "minute").subtract(BeforeTimeHour, "hours")
            .subtract(BeforeTimeDay, "days").subtract(BeforeTimeMonth, "months").subtract(BeforeTimeYear, "years").format("YYYY-MM-DD HH:mm:ss")
        },
                ...
                if (PressButtonList.length != 0) {
                    for (let j = 0; j < PressButtonList.length; j++) {
                        if (PressButtonList[j].ElementName ==this.textblockData[i].Name) {
                             let AssociateDateTimePickerList = this.data.Data.AssociateDateTimePickerList
                             this.TimePickerListFun(AssociateDateTimePickerList,this.textblockData[i].Name)
                              //渐变色
                            ...
                            this.dataValue.push(value);
                             ...
                             setTimeout(()=>{
                                    var dom = document.querySelector(`.${this.textblockData[i].Name}picker`)
                                    ...
                                    dom.querySelector(`.el-input__inner`).style.boxShadow=colorData.Shadow2
                                    dom.querySelector(`.el-input__inner`).style.fontSize=this.textblockData[i].PropertyList.FontSize+'px'
                                    let TextDecorations = this.textblockData[i].PropertyList.TextDecorations == 'False' ? 'none' : 'underline'
                                    dom.querySelector(`.el-input__inner`).style.textDecoration=TextDecorations
                                    
                                    dom.querySelector(`.el-input__inner`).style.fontFamily=this.textblockData[i].PropertyList.FontFamily

                                    dom.querySelector(`.el-input__prefix .el-input__icon`).style.setProperty('line-height', `${BorderThickness}px`, 'important');
                                },300)
                            ...
                            if (j == PressButtonList.length - 1) {
                                ...
                                this.dataValue.push(value5);
                                  ...
                                  setTimeout(()=>{
                                    ...
                                    dom.querySelector(`.el-input__inner`).style.color=colorData.foreColor
                                    dom.querySelector(`.el-input__inner`).style.boxShadow=colorData.Shadow2
                                    dom.querySelector(`.el-input__inner`).style.fontSize=this.textblockData[i].PropertyList.FontSize
                                    let TextDecorations = this.textblockData[i].PropertyList.TextDecorations == 'False' ? 'none' : 'underline'
                                    dom.querySelector(`.el-input__inner`).style.textDecoration=TextDecorations

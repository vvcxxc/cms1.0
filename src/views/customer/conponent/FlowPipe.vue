            ...
            <div
                ...
                :style="
                    'width:' +
                        ...
                        item.Shadow
                "
                @dblclick="opendb(item, $event)"
            >
                <div class="flowpipediv" :class="{ fz: item.fz }">
                    <div
                        class="flowline"
                        v-for="(item1, index1) in item.intervalarr"
                        :key="index1"
                        :style="
                            'background:' +
                                item.lineColor +
...
export default {
    ...
    methods: {
        ...
        axioImg2(resArr) {
            ...
            if(this.isRealNum(resArr1.value)){
                resArr1.value = Number(resArr1.value)
            }
                ...
                this.data.Data.FlowPipeAnimationList.forEach(item1 => {
                    if (item.class == item1.ElementName) {
                        ...
                        if (item1.LineTagName == resArr1.name) {
                            //    if(item.class=='FlowPipe130_f6f0cadc_77d2_413a_8845_99a9b70ea21b'){
                            //      debugger
                            //  }
                            // if (resArr.name == 'Y2') {
                            // }
                            if (
                                item.intervalarr.length > 0 &&
                                resArr1.value == 'False'
                            ) {
                                item.isflow = true;
                                item.intervalarr = [];
                        ...
                        if (
                            item1.SpeedTagName == '0' ||
                            item1.SpeedTagName == 'False' ||
                            item1.SpeedTagName == 'false'
                        ) {
                            if (item.isflow) {
                                clearInterval(item.time);
                            ...
                            for (
                                let i = 0;
                                i < item1.FlowPipeLineAnimationList.length;
                                i++
                            ) {
                                let showCompare = JSON.parse(
                                    JSON.stringify(
                                        item1.FlowPipeLineAnimationList[i]
                                            .Compare
                                    )
                                );
                                if (
                                ...
                                // console.log(showCompare)
                                if (parseFloat(showCompare) == parseFloat(showCompare1)) {
                                    let one = item1.FlowPipeLineAnimationList[
                                        i
                                    ].Color.slice(1, 3);
                                    let two = item1.FlowPipeLineAnimationList[
                                        i
                                    ].Color.split('');
                                    two.splice(1, 2);
                                    two = two.join('');
                                    ...
                                    if (
                                        item1.FlowPipeLineAnimationList[i].Color
                                            .length >= 8
                                    ) {
                                        item.lineColor = two + one;
                                    } else {
                                        item.lineColor =
                                            item1.FlowPipeLineAnimationList[
                                                i
                                            ].Color;
                                    }
                                } else {
                                    item.lineColor = JSON.parse(
                                        JSON.stringify(item.lineColor2)
                                    );
                                }
                            }
                        } else {
            ...
            this.dataValue.forEach((item, index) => {
                ...
                if (item.isflow) {
                    if (!item.hasOwnProperty('time')) {
                        if (item.FlowSpeed1) {
                            item.FlowSpeed = JSON.parse(
                                JSON.stringify(item.FlowSpeed1)
                            );
                        }
                        console.log(item);
                        if (
                        ...
                        if (
                            parseInt(item.FlowSpeed) !== 99999999999999999999 &&
                            parseInt(item.FlowSpeed) !== 100000000000000000000
                        ) {
            ...
            this.$axios({
                ...
                .then(res => {
                    res.data.data.forEach((item)=>{
                        if(this.isRealNum(item.Value)){
                         item.Value = Number(item.Value)
                        }
                    })
                    let res1 = res.data.data.filter(
                        item => item.Value !== '???'
                    );
                        ...
                        this.data.Data.FlowPipeAnimationList.forEach(item1 => {
                            if (item.class == item1.ElementName) {
                                if (
                                    !isNaN(
                                        JSON.parse(
                                            JSON.stringify(item1.SpeedTagName)
                                        )
                                    )
                                ) {
                                    item.FlowSpeed1 = JSON.parse(
                                        JSON.stringify(item1.SpeedTagName)
                                    );
                                }
                                console.log(
                                ...
                                if (
                                    ...
                                    item.FlowSpeed = String(
                                        parseInt(item.FlowSpeed)
                                    );
                    ...
                    if (res1.length !== 0) {
                        this.dataValue.forEach(item => {
                            this.data.Data.FlowPipeAnimationList.forEach(
                                item1 => {
                                    if (item.class == item1.ElementName) {
                                        res1.forEach(item2 => {
                                            if (
                                                item1.SpeedTagName == item2.Name
                                            ) {
                                                item.FlowSpeed = item2.Value;
                                                item.FlowSpeed1 = item2.Value;
                                            }
                                            if (
                                                item1.LineTagName == item2.Name
                                            ) {
                    ...
                    this.dataValue.forEach((item, index) => {
                        ...
                        if (
                            ...
                            item.time = setInterval(() => {
                                if (
                                    item.FlowSpeed == 99999999999999999999 ||
                                    item.FlowSpeed == 100000000000000000000
                                ) {
                                    clearInterval(item.time);
                                    return;
                                }
        ...
        jurisdictionShow(item) {
            return new Promise((resolve, reject) => {
                ...
                if (JSON.parse(sessionStorage.getItem('userInfo1')) == null) {
                    ...
                    userinfoID = JSON.parse(sessionStorage.getItem('userInfo1'))
                        .SCMSUserID;
                }
        ...
        gitValue() {
            ...
            for (var i = 0; i < this.textblockData.length; i++) {
                ...
                var LineStroke = JSON.parse(
                    this.textblockData[i].PropertyList.LineStroke
                );
                ...
                if (LineStroke.ColorType == 'SolidColor') {
                    lineColor =
                        '#' +
                        LineStroke.Data.Color.slice(3) +
                        LineStroke.Data.Color.slice(1, 3);
                } else {
                    lineColor = '';
                    lagel2 = LineStroke.Data.Angel.toFixed(0);
                    for (
                        var h3 = 0;
                        h3 < LineStroke.Data.GradientStops.length;
                        h3++
                    ) {
                ...
                for (
                    let i1 = 0;
                    i1 <
                    Math.ceil(this.textblockData[i].PropertyList.Width / 48) +
                        2;
                    i1++
                ) {
                    intervalarr.push((i1 - 1) * 48);
                ...
                if (
                    this.textblockData[i].PropertyList.FlowSpeed === 'True' ||
                    this.textblockData[i].PropertyList.FlowSpeed === 'true'
                ) {
                    this.textblockData[i].PropertyList.FlowSpeed = '2';
                }
                this.textblockData[i].PropertyList.FlowSpeed = String(
                    parseInt(this.textblockData[i].PropertyList.FlowSpeed)
                );
                ...
                if (this.textblockData[i].PropertyList.FlowSpeed === '3') {
                    ...
                    this.textblockData[i].PropertyList.FlowSpeed === '1'
                ) {
                    FlowSpeed = 2000;
                    fz = false;
                } else if (
                    this.textblockData[i].PropertyList.FlowSpeed === '0' ||
                    this.textblockData[i].PropertyList.FlowSpeed === 'NaN'
                ) {
                    ...
                    this.textblockData[i].PropertyList.FlowSpeed === '-3'
                ) {
                    FlowSpeed = 400;
                    fz = true;
                }

...
<template>
    <div
        :style="`width:${item.width}px;height:${item.height}px;position:absolute;left:${item.left}px;top:${item.top}px;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                zIndex:0;opacity:${item.opacity}; transform:rotate(${item.rotate}deg);zIndex:${item.ZIndex}`">
        <div ref="LineChart22" @dblclick="opendb(item)" @contextmenu.prevent @mousedown="downClick(item, $event)"
            @mouseup="seupClick(item, $event)" :style="`width:100%;height:100%;`">22 </div>

        <!-- 这线图添加滑块需求 -->
        <div class="slider">
            <div class="slider_reset" title="点击重置滑块" @click="resetSlider"></div>
            <div class="slider_mode">
                <el-checkbox v-model="sliderData.show" label="滑块模式" @change="changeSllider" size="small" />
            </div>
            <div class="slider_count">
                <!-- <span>数据条数</span> -->
                <el-input-number v-model="sliderData.count" :disabled="!sliderData.show" :min="1"
                    :id="`${data.name}sliderInput`" :controls="false" @blur="blueSliderInput"
                    @keyup.enter.native="enterSliderInput(`${data.name}sliderInput`)" class="slide_count_input" />
                <el-tooltip effect="dark" content="图表显示的数据条数" placement="bottom-start">
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            /* 新增需求 -滑块模式 */
            let sessionSliderData = JSON.parse(sessionStorage.getItem(`${this.data.id}_${this.data.name}`))
            if (sessionSliderData !== null /* === this.data.name */) {
                // 缓存滑块数据
                this.sliderData.show = sessionSliderData.show;
                this.sliderData.startValue = 0;
                // this.sliderData.startValue = this.optionData.Datas.XData.length - sessionSliderData.count >= 0 ? this.optionData.Datas.XData.length - sessionSliderData.count : 0
                this.sliderData.count = sessionSliderData.count
            } else {
                if (!this.optionData.DataZoom) {
                    this.optionData.DataZoom = {
                        Show: false,
                        DataNumber: 5
                    }
                }
                this.sliderData.show = this.optionData.DataZoom.Show /* true */
        ...
        enterSliderInput(id) {
            this.$nextTick(() => {
                ...
                input.blur()
                this.blueSliderInput()
            })
        },
        blueSliderInput() {
            let chart = this.myChart.getOption()
            if (this.sliderData.count > chart.xAxis[0].data.length) {
                this.sliderData.count = chart.xAxis[0].data.length
            }
                        ...
                        if (seData1.length == 0) {
                            ...
                            seData1.forEach((item1) => {
                                if (item1.name == item.yAxisIndex) {
                                    ...
                                    item1.data = [...item1.data, ...item.data]
                                } else {
                                    seData2 = [];
                                    let data = {
                                        name: item.yAxisIndex,
                        ...
                        Ddata.yAxis.forEach((item, key) => {
                            ...
                            if (item.Adaptive || YAxisMax1 == null || YAxisMin1 == null) {
                                ...
                                if (Number(YAxisMin1) > parseFloat(Math.min.apply(null, data))) {
                                    console.log()
                                    YAxisMin1 = this.getarea(parseFloat(Math.min.apply(null, data)), 'min', zf)
                                }
                                let zf1;
                                zf1 = Math.max.apply(null, data) >= 0 ? 1 : -1;
                                if (Number(YAxisMax1) < parseFloat(Math.max.apply(null, data))) {
                                    YAxisMax1 = this.getarea(parseFloat(Math.max.apply(null, data)), 'max', zf1)
                                }
                                Adaptive1 = false
                            }
                            ...
                            if (this.$refs.LineChart22) {
                                ...
                                if ($this.sliderData.show) {
                                        ...
                                        let dataZoom = [
                                            {
                                                ...
                                                height: 15,
                                                bottom: 10,
                                                startValue: Ddata.xAxis[0].data.length - $this.sliderData.count > 0 ? Ddata.xAxis[0].data.length - $this.sliderData.count : 0,
                                                endValue: Ddata.xAxis[0].data.length - 1
                                            }
                                        ]

                                        $this.sliderData.startValue = Ddata.dataZoom[0].startValue
                                    ...
                                    Ddata.dataZoom = [{
                                        ...
                                        height: 15,
                                        bottom: 10,
                                        startValue: Ddata.xAxis[0].data[0],
        ...
        drawLine() {
            ...
            this.$nextTick(() => {
                ...
                var aa = {
                    ...
                    "toolbox": {
                        ...
                        "feature": {
                            ...
                            //     onclick: () => {
                            //         let _chart = this.$echarts.init(this.$refs.LineChart22);
                ...
                if ($this.sliderData.show) {
                    aa.dataZoom[0].startValue = aa.xAxis.data[$this.sliderData.startValue]
                    aa.dataZoom[0].endValue = aa.xAxis.data[$this.sliderData.startValue + $this.sliderData.count - 1]
                    sessionStorage.setItem(`${$this.data.id}_${$this.data.name}`, JSON.stringify(this.sliderData))
                }
                ...
                if (this.myChart) {
                    ...
                    this.myChart.on('datazoom', function (params) {
                        ...
                        let endValue = $this.myChart.getModel().option.dataZoom[1].endValue // x轴结束值
                        $this.sliderData.startValue = startValue
...
.tip_icon {
    ...
    height: 20px;
    margin-left: 3px;
    background: url('../../../assets/images/prompt.png');
...
::v-deep .slide_count_input.el-input-number {
    width: 40px;
    margin-left: 5px;
...
::v-deep .slide_count_input.el-input-number .el-input.is-disabled .el-input__inner {

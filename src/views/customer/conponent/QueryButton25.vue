 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-04-07 11:00:25
 -->
  ...
  methods: {
    init(){
      ...
      for(let i=0;i<this.dataValue.length;i++){
          ...
          console.log("u",this.dataValue.length)
              console.log("u",this.dataValue)
        let i1 = false
        if(i == this.dataValue.length-1){
          ...
          this.searchData(this.dataValue[i],1,i1)
      }
    },
          ...
          return new Promise((resolve, reject) => {
            var userinfoID
              if(JSON.parse(sessionStorage.getItem('userInfo1')) == null){
                if(sessionStorage.getItem('sightseerInfo1') != null){

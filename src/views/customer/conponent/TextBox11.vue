   ...
   <div v-for="(item,index) in dataValue" :key="index">
      ...
      @dblclick="opendb(item)" @contextmenu.prevent @mousedown="downClick(item,$event)" @mouseup="seupClick(item,$event)"
       class="TextBox11aa contenBox" :style="'width:' + item.width + 'px; height:' 
          ...
          + item.Shadow + ';'">
            <input autocomplete="off"  @keyup.enter="keupFun(item)" name="datatextbox" :class="item.class" v-model="item.text"
             ...
             + item.Foreground+';fontWeight:'+item.Blod + ';text-decoration:'+item.TextDecorations + ';fontSize:'+item.FontSize + 'px'" type="text" />
          </div>
      </div>

...
<template>
  <div>
    <div class="DateTimePicker29a" v-for="(item, index) in dataValue" :key="index">
      <div :class="item.class" @mouseup="upFun(item, $event)" @mousedown="downFun(item, $event)" @dblclick="opendb(item)"
        @contextmenu.prevent name="combinecombobox" :style="'position:absolute; left:' + item.left + 'px; top:' + item.top
        ...
        <el-select @focus="selectClick($event, item)" v-model="item.value" clearable
          ...
          <el-option :style="'width:100%;height:100%;'" v-for="it in item.options" :key="it.value" :label="it.label"
            :value="it.value">
          </el-option>
        </el-select>
      </div>
...
export default {
  ...
  methods: {
              ...
              $this.dataValue[i].options1.forEach((item, key) => {
                ...
                ss.forEach((item1) => {
                  if (res.name == item1.TagName) {
                    ...
                    } else if (type == $this.lang['二进制变量'] || type == $this.lang['字符串']) {
                      if (item.Data == res.value) {
                        if (!item.IsDefault) {
                          $this.dataValue[i].value = item.Name
                        }
                      }
    ...
    init() {
      ...
      //  console.log("this.textblockData",dataAll)
    ...
    gitValue() {
      ...
      for (let i = 0; i < this.textblockData.length; i++) {
        console.log("zzz", this.textblockData[i])
        //加层级zIndex
        ...
        for (let o1 = 0; o1 < this.dataValue.length; o1++) {
          ...
          if (this.data.Data.CombineComboBoxList[o].ElementName == this.dataValue[o1].class) {
            ...
            this.data.Data.CombineComboBoxList[o].SoucesItems.forEach((item) => {
              ...
              if (item.Name == '整型16') {
                console.log('item', this.data.Data.CombineComboBoxList[o].SoucesItems)
              }

<!--
 * @Description: 这是导出按钮页面（组件）
 * @Date: 2020-05-14 16:23:20
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-01-25 16:40:40
 -->
...
<div>
  ...
  <div
      v-if="show" 
       ...
       @click="exportdata(item,$event)" 
      name="outputbutton"  :class="item.class" class="outputbutton26a" 
      ...
      + item.rotate + 'deg);padding:' + item.BorderThickness + 'px;overflow:hidden;white-space:nowrap;boxShadow:'
      ...
      <div class="conten" :style="'width:100%;height:100%;background:' + item.Background + ';borderRadius:' 
        ...
        <div class="pressButton_box 11"></div>
  </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    exportdata(it,e){
       this.jurisdictionShow(it).then(val => { 
          if(this.CanExcuteShow){
            // this.commerPopShow1 = true
            this.$emit('showtip',this.lang.NoOperationAuthority)
          }else{
             this.$emit('exportdata',it);
          }
       })
    },
...
.outputbutton26a:hover{
  background-color: #71A2C7ff !important;
  background: #71A2C7ff !important;
}

.outputbutton26a:hover .conten{
  background-color: #71A2C7ff !important;
  background: #4EBAF3ff !important;
}
.outputbutton26a:active{
  background-color: #71A2C7ff !important;
  background: #71A2C7ff !important;

}
.outputbutton26a:active .conten{

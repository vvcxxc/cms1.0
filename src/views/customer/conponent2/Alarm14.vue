   ...
   <div class="alarmBox">
    <div v-for="(item,index) in dataValue" :key="index">
        ...
        v-if="show"
         ...
         name="alert"
         :class="'aa'+item.Class" class="alarm"
         ...
         +item.rotate+'deg);padding:'+item.WraperBorderThickness+'px;overflow:hidden;white-space:nowrap;background:'+item.BorderBrush">
            ...
            +item.radiusButton+'px '+item.radiusLeft+'px;padding:14px;box-sizing:border-box;background:'+item.Background">
                ...
                +'px;overflow:hidden'">
                    <div :class="'aa'+item.Class+'AlarmTitle'" style="position:absolute;left:0px">
                ...
                <div :class="'aa'+item.Class+'alarm_conter'" class="alarm_conter"
                   ...
                   +'overflow-x:hidden;position: relative;background:'+item.Background">
                    <div :class="'aa'+item.Class+'alarm_conter_rowBox'" class="alarm_conter_rowBox" 
                        ...
                        :key="inde" :class="['aa'+item.Class+'alarm_conter_row',inde==0?'active21':'']"
                         class="alarm_conter_row" name="alarm_conter_row" :style="'background:'+item.itembackColor+';border-bottom:1px solid;border-color:#090D2C;display:flex'">
                             ...
                             <!-- active21 默认不激活-->
                             <div class="alarm_conter_no" :class="['aa'+routerName+item.Class+'alarm_conter_no',inde==0?'active21':'']" name="no"
                ...
                <div class="alarm_fool clearfix" :class="'aa'+item.Class+'alarm_fool'" style="margin-top:14px">
                    <div @mousedown="leftDownFun(item)" @mouseup="leftUpFun(item)" :class="'aa'+item.Class+'alarm_fool_left'" class="alarm_fool_left left"></div>
                    <div  @mousedown="rightDownFun(item)" @mouseup="rightUpFun(item)" :class="'aa'+item.Class+'alarm_fool_right'" class="alarm_fool_right left"></div>
...
export default {
    ...
    //加入报警控件变量
     JoinVarFun(){
              ...
              this.msgHub2 = window.$.connection.redisMonitor
                if(this.msgHub2){
                    for(let i=0;i<this.dataValue.length;i++){
                            ...
                            console.log('wweee',window.$.connection.redisMonitor.server)
        ...
        //点击选择事件
        alarmRowFun(item,e){
            var rowdom = document.querySelectorAll(`.aa${item.Class}alarm_conter_row`)
            ...
            var name = $(e.path[0]).attr('name')
               if(name == 'alarm_conter_row'){
                   ...
                   $(e.path[0]).parent(`.aa${item.Class}alarm_conter_row`).addClass('active1')
                   $(e.path[0]).parent(`.aa${item.Class}alarm_conter_row`).find(`.alarm_conter_text`).addClass('active2')
                   $(e.path[0]).parent(`.aa${item.Class}alarm_conter_row`).find(`.alarm_conter_no`).addClass('active2')
               }
        },
        //右按下
        rightDownFun(item){
            this.time1 = setInterval(()=>{
                var dom = document.querySelector(`.aa${item.Class}alarm_conter_rowBox`).offsetWidth
                var dom1 = document.querySelector(`.aa${item.Class}alarm_conter`).offsetWidth
                var left = parseInt(document.querySelector(`.aa${item.Class}alarm_conter_rowBox`).style.left)
                var num = dom1 - dom
                if(left>num){
                    document.querySelector(`.aa${item.Class}alarm_conter_rowBox`).style.setProperty('left',`${left - 1}px`,'important')
                    document.querySelector(`.aa${item.Class}AlarmTitle`).style.setProperty('left',`${left - 1}px`,'important')
                }
            },10)
        },
        //右松开
           ...
           for(let k=0;k<this.dataValue.length;k++){
               ...
               var vv = {
                ...
                this.$axios({
                    ...
                    console.log('this.dataValue[k]',res.data.data,this.dataValue[k])
        ...
        datafun(resData,index,className,item){
                      ...
                      for(var j=0;j<this.dataArr.length;j++){
                        ...
                        this.dataValue[index].num = this.dataValue[index].resdata.length
                         //横向箭头是否出现
                         var alarm_conter = document.querySelector(`.aa${className}alarm_conter`).offsetWidth
                         var alarm_conter_rowBox =document.querySelector(`.aa${className}alarm_conter_rowBox`).offsetWidth
                         var res = alarm_conter_rowBox > alarm_conter
                         if(res == true){
                             document.querySelector(`.aa${className}alarm_fool_left`).style.display = 'block'
                             document.querySelector(`.aa${className}alarm_fool_right`).style.display = 'block'
                         }else{
                             document.querySelector(`.aa${className}alarm_fool_left`).style.display = 'none'
                             document.querySelector(`.aa${className}alarm_fool_right`).style.display = 'none'
                         }

                        
                         //排序字段dom宽度变为一致
                         var noDom = document.querySelectorAll(`.aa${this.routerName}${className}alarm_conter_no`)
                         var noDomLength = 0
                         ...
                         var titleDom = document.querySelector(`.aa${this.routerName}${className}alarm_title_no`)
                        ...
                        var headH = document.querySelector(`.aa${className}AlarmTitle`).offsetHeight
                        var flooH = document.querySelector(`.aa${className}alarm_fool`).offsetHeight

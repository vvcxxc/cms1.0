...
<div>
  <div v-for="(item,index) in dataValue" :key="index">
    ...
    + item.zindex + ';boxShadow:' + item.Shadow +';zIndex:'+item.ZIndex + `;border: ${item.showBorder ? `${item.StrokeThickness}px solid ${item.borderStyle}` : ''}`  ">
  ...
  this.$axios({
    ...
    async AxiosFlowList(){
      ...
      axioImg2(resArr,first){
          ...
          if(resArr.value == 'true' || resArr.value == '1' || resArr.value == true){

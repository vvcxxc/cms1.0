 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-02-24 11:29:47
 -->
<template>
    <div>
        <div v-for="(item, index) in dataValue" :key="index">
            ...
            >
                <div
                    class="box"
                    :style="
                        'position:absolute;zIndex:10;width:' +
                        item.width +
                        'px;height:' +
                        item.height +
                        'px;boxSizeng:border-box;-webkit-mask-repeat: repeat-x, repeat-y, repeat-x, repeat-y;' +
                        '-webkit-mask-position: 0 0, 0 0, 0 100%, 100% 0;zIndex:9;-webkit-mask-image: linear-gradient' +
                        '(to right, #000 ' +
                        item.StrokeThickness +
                        'px, transparent ' +
                        item.StrokeThickness +
                        'px), linear-gradient(to bottom, #000 ' +
                        item.StrokeThickness +
                        'px, transparent ' +
                        item.StrokeThickness +
                        'px),' +
                ...
                <div
                    ...
                    :style="
                        'width:' +
                        (item.width - item.StrokeThickness * 2) +
                        'px;zIndex:9;height:' +
                        (item.height - item.StrokeThickness * 2) +
                        'px;position:absolute;left:0;top:0;zIndex:10;margin-top:' +
                        item.StrokeThickness +
                        'px;margin-left:' +
                        item.StrokeThickness +
                        'px;background:' +
                    ...
                    >
                        <div
                            :style="
                                ...
                                'px'
                            "
                            v-for="(it, ind) in item.ProductFlowList"
                            :key="ind"
                        >
                            <div
                                class="product_Box"
                                :style="
                                    ...
                                    'px ' +
                                    i.ColuPadding +
                                    'px;position:relative;text-align:conter;white-space:nowrap;'
                                "
                                v-for="(i, inde) in it"
                                :key="inde"
                            >
                                ...
                                >
                                    <div
                                        v-for="(ii, indd) in i.Arr"
                                        :key="indd"
                                    >
                                        {{ ii.Name }}:{{ ii.Value }}
                                    </div>
                                    <div
                                        class="suspensionShow"
                                        :style="
                                            'position:absolute;bottom:' +
                                            (i.Direction == 'Bottom'
                                                ? '5px'
                                    ...
                                    >
                                        ...
                                        <div
                                            :style="
                                                'background:#727272' +
                                                ';color:#fff' +
                                                ';fontSize:12px;float:left;padding:10px;border-radius:4px;text-align:conter;overflow:hidden;white-space:nowrap;'
                                            "
                                        >
                                            <div
                                                v-for="(ii, indd) in i.Arr"
    ...
    methods: {
        ...
        moveFun(name, value, resArr) {
            ...
            for (let k1 = 0; k1 < this.dataValue.length; k1++) {
                if (name == this.dataValue[k1].EntryVariable) {
                    ...
                    let boxdom = document.querySelector(
                        `.${domName}`
                    ).offsetHeight;
                    ...
                    this.dataValue[j].setTime = setInterval(() => {
                        for (let f = 0; f < listArr.length; f++) {
                            if (listArr[f][0].Direction == 'Bottom') {
                                if (
                                    listArr[f][0].Bottom >= this.boxdomArrH[j]
                                ) {
                                    listArr.splice(f, 1);
                                } else {
                                    listArr[f][0].Bottom =
                                        listArr[f][0].Bottom +
                                        this.dataValue[j].SpeedVariableP.value;
                                }
                            }
                            if (listArr[f] != undefined) {
                                if (listArr[f][0].Direction == 'Top') {
                                    if (
                                        listArr[f][0].Top >= this.boxdomArrH[j]
                                    ) {
                                        listArr.splice(f, 1);
                                    } else {
                                        listArr[f][0].Top =
                                            listArr[f][0].Top +
                                            this.dataValue[j].SpeedVariableP
                                                .value;
                                    }
                                }
                            }
                            if (listArr[f] != undefined) {
                                if (listArr[f][0].Direction == 'Left') {
                                    if (
                                        listArr[f][0].Left >= this.boxdomArrW[j]
                                    ) {
                                        listArr.splice(f, 1);
                                    } else {
                                        listArr[f][0].Left =
                ...
                if (dataProductFlowList[k].IsDynamic) {
                    ...
                    for (let k1 = 0; k1 < FlowList.length; k1++) {
                        if (FlowList[k1].Info.VariableName != '') {
                            this.VariableArr.push(
                                FlowList[k1].Info.VariableName
                            );
                        }
                    }
        ...
        judgeFun(data, name) {
            for (let k = 0; k < this.dataValue.length; k++) {
                ...
                let index = this.VariableArr.indexOf(
                    this.dataValue[k].SpeedVariableP.name
                );
                if (index != -1) {
                    var resV = Number(
                        data[index].Value == null ? 0 : data[index].Value
                    );
                    this.dataValue[k].SpeedVariableP.value =
                        (this.dataValue[k].Target / this.dataValue[k].Source) *
                        resV *
                        0.5;
                }
                if (this.dataValue[k].EntryVariable == name) {
                    for (let k1 = 0; k1 < ProductFlowList.length; k1++) {
                        ...
                        for (let k2 = 0; k2 < Arrayl.length; k2++) {
                            ...
                            let index2 = this.VariableArr.indexOf(
                                Arrayl[k2].模拟高度.Name
                            );
                            let index3 = this.VariableArr.indexOf(
                                Arrayl[k2].模拟宽度.Name
                            );

                            if (
                                Arrayl[k2].模拟高度.Name == '' ||
                                Arrayl[k2].模拟高度.Name == null ||
                                Arrayl[k2].模拟高度.Name == 'null'
                            ) {
                                this.dataValue[k].ProductFlowList[k1][
                                    k2
                                ].模拟高度.Value = Arrayl[k2].DefaultH;
                            } else {
                                if (index2 != -1) {
                                    var resH =
                                        data[index2].Value == null
                                            ? 0
                                            : data[index2].Value;
                                    this.dataValue[k].ProductFlowList[k1][
                                        k2
                            ...
                            if (
                                Arrayl[k2].模拟宽度.Name == '' ||
                                Arrayl[k2].模拟宽度.Name == null ||
                                Arrayl[k2].模拟宽度.Name == 'null'
                            ) {
                                this.dataValue[k].ProductFlowList[k1][
                                    k2
                                ].模拟宽度.Value = Arrayl[k2].DefaultW;
                            } else {
                                if (index3 != -1) {
                                    var resW =
                                        data[index3].Value == null
                                            ? 0
                                            : data[index3].Value;
                                    this.dataValue[k].ProductFlowList[k1][
                                        k2
            ...
            if (this.VariableArr) {
                ...
                if (index != -1) {
                    for (let i = 0; i < this.resData[0].length; i++) {
                        if (
                            this.resData[0][i].Name == this.VariableArr[index]
                        ) {
                            resShow =
                                this.resData[0][i].Value.toString() ==
                                resArr.value.toString()
                                    ? false
                                    : true;
                        }
                    }
        ...
        gitValue() {
            ...
            for (let i = 0; i < this.textblockData.length; i++) {
                ...
                //渐变色变量
                var FilldArr = JSON.parse(
                    this.textblockData[i].PropertyList.Fill
                );
                var StrokeArr = JSON.parse(
                    this.textblockData[i].PropertyList.Stroke
                );
                var lagel2;
                var gradient1;
                var lagel3;
                ...
                if (FilldArr.ColorType == 'SolidColor') {
                    fillColor =
                        '#' +
                        FilldArr.Data.Color.slice(3) +
                        FilldArr.Data.Color.slice(1, 3);
                    if (
                        FilldArr.Data.Color.slice(3) == 'FFFFFF' &&
                        FilldArr.Data.Color.slice(1, 3) != 'FF'
                    ) {
                        strokeColor =
                            '#FFFFFF' + FilldArr.Data.Color.slice(1, 3);
                    }
                } else {
                    fillColor = '';
                    ...
                    for (
                        var h2 = 0;
                        h2 < FilldArr.Data.GradientStops.length;
                        h2++
                    ) {
                ...
                if (dataProductFlowList.length) {
                    for (let k = 0; k < dataProductFlowList.length; k++) {
                        //流动控件开始
                        if (
                            dataProductFlowList[k].ElementName ==
                            this.textblockData[i].Name
                        ) {
                            if (dataProductFlowList[k].IsDynamic) {
                                var CorrelationInfos =
                                    dataProductFlowList[k].CorrelationInfos;
                                var ProductIdArr = [];
                                for (
                                    let k1 = 0;
                                    k1 < CorrelationInfos.length;
                                    k1++
                                ) {
                                    ProductIdArr.push(
                                        CorrelationInfos[k1].ProductId
                                    );
                                }
                                ...
                                for (
                                    ...
                                    for (
                                        var k3 = 0;
                                        k3 < CorrelationInfos.length;
                                        k3++
                                    ) {
                                        if (
                                            ProductIdArr[k2] ==
                                            CorrelationInfos[k3].ProductId
                                        ) {
                                            let name =
                                                CorrelationInfos[k3].Info
                                                    .Remarks;
                                            let name1 =
                                                CorrelationInfos[k3].Info
                                                    .VariableName;
                                            ...
                                            if (name == '模拟宽度') {
                                                value =
                                                    dataProductFlowList[k]
                                                        .SizeInfo
                                                        .SimulationWidth.Default
                                                        .Value;
                                            ...
                                            ProductIdArr4.FontColor =
                                                dataProductFlowList[
                                                    k
                                                ].FontColor.HtmlColor;
                                            ProductIdArr4.ProductColor =
                                                dataProductFlowList[
                                                    k
                                                ].ProductColor.HtmlColor;
                                            ProductIdArr4.StartPointX =
                                                dataProductFlowList[
                                                    k
                                                ].StartPoint.X.Value;
                                            ProductIdArr4.StartPointY =
                                                dataProductFlowList[
                                                    k
                                                ].StartPoint.Y.Value;
                                            ProductIdArr4.ColuPadding =
                                                dataProductFlowList[
                                                    k
                                                ].Interval.ColumnInterval.Value;
                                            ProductIdArr4.RowPadding =
                                                dataProductFlowList[
                                                    k
                                                ].Interval.RowInterval.Value;
                                            ProductIdArr4.DefaultH =
                                                dataProductFlowList[
                                                    k
                                                ].SizeInfo.SimulationHeight.Default.Value;
                                            ProductIdArr4.DefaultW =
                                                dataProductFlowList[
                                                    k
                                                ].SizeInfo.SimulationWidth.Default.Value;
                                            ProductIdArr4.ProductId =
                                                CorrelationInfos[k3].ProductId;
                                            ProductIdArr4.FontSize =
                                                dataProductFlowList[k].FontSize;
                                            ProductIdArr4.Arr = [];
                                            ProductIdArr4.Source =
                                                dataProductFlowList[
                                                    k
                                                ].Scale.Source.Value.Value;
                                            ProductIdArr4.Target =
                                                dataProductFlowList[
                                                    k
                                                ].Scale.Target.Value.Value;
                                            ProductIdArr4.Bottom =
                                                dataProductFlowList[k]
                                                    .Direction == 'Bottom'
                                                    ? 0
                                                    : 'none';
                                ...
                                var column =
                                    dataProductFlowList[k].Layout.Column.Value;
                                var Row =
                                    dataProductFlowList[k].Layout.Row.Value;
                                var columnNum = 0;
                                let ProductIdArr5 = [];
                                //向上向下
                                if (
                                    dataProductFlowList[k].Direction == 'Top' ||
                                    dataProductFlowList[k].Direction == 'Bottom'
                                ) {
                                    for (
                                        let k4 = 0;
                                        k4 < ProductIdArr3.length / column;
                                        k4++
                                    ) {
                                        ProductIdArr5.push(
                                            ProductIdArr3.slice(
                                ...
                                var value = {
                                    ...
                                    StartPointY:
                                        dataProductFlowList[k].StartPoint.Y
                                            .Value,
                                    StartPointX:
                                        dataProductFlowList[k].StartPoint.X
                                            .Value,
                                    Bottom:
                                        dataProductFlowList[k].Direction ==
                                        'Bottom'
                                            ? 0
                                            : 'none',
                                    Top:
                                    ...
                                    EntryVariable:
                                        dataProductFlowList[k].EntryVariable,
                                    SpeedVariableP: SpeedVariable,
                                    EntryShow: 'false',
                                    suspensionShow: false,
                                    setTime: k,
                                    Source: dataProductFlowList[k].Scale.Source
                                        .Value.Value,
                                    Target: dataProductFlowList[k].Scale.Target
                                        .Value.Value,
                                };
                    ...
                    for (let o = 0; o < this.dataValue.length; o++) {
                        this.dataFiArr.push(
                            this.dataValue[o].ProductFlowList[0]
                        );
                    }
                    ...
                    var value1 = {
                        ...
                        Stroke: strokeColor,
                        StrokeThickness: Number(
                            this.textblockData[i].PropertyList.StrokeThickness
                        ),
                        soDa:
                            this.textblockData[i].PropertyList.StrokeDashArray
                                .length == 0
                                ? 1
                                : this.textblockData[i].PropertyList
                                      .StrokeThickness * 5,

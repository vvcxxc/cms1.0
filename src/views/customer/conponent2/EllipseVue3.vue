 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-01-26 09:29:11
 -->
<template>
<div>
    <div v-for="(item,index) in dataValue" :key="index">
    <!-- <div v-if="show" class="Ellipse3aa2" :style="'width:'+ item.width +'px; height:'+ item.height +'px; borderRadius:' + big 
    + '%; border:' + item.StrokeThickness + 'px ' + item.soDa + item.Stroke + '; backgroundColor:' 
    + item.Fill + '; position:absolute; top:' + item.top + 'px; left:' + item.left + 'px; opacity:' 
    + item.opacity + '; transform:rotate('+ item.rotate + 'deg)'"></div>-->
    
    <div v-if="show" 
    @dblclick="opendb(item)" @contextmenu.prevent @mousedown="downClick(item,$event)" @mouseup="seupClick(item,$event)"
    :class="item.class" class="Ellipse3aa2" :style="'width:'+ item.width +'px; height:'+ item.height 
    +'px; borderRadius:' + big + '%; position:absolute; top:' + item.top + 'px; left:' + item.left 
    + 'px; opacity:' + item.opacity + '; transform:rotate('+ item.rotate + 'deg);'" name="solidellipse">
    <div class="aa" :style="'width:100%;height:100%;position:absolute;borderRadius:50%;boxShadow:' 
    + item.Shadow + ';zIndex:'+item.ZIndex">
    </div>

        <svg :width="item.width" :height="item.height">
          <defs>
              <linearGradient :id="'gradrr' +item.class "
              ...
              <linearGradient :id="'dasharr' + item.class"
            ...
            <ellipse id="myellipse" :cx="(item.width - (item.StrokeThickness * 2) )/2 + Number(item.StrokeThickness)" 
                    ...
                    :fill="'url(#gradrr' + item.class + ')'"  :stroke-dasharray="item.soDa"
                    :stroke="'url(#dasharr' + item.class + ')'" 
...
export default {
    ...
    methods: {
        ...
        reSvg(){
          console.log('qqccaa',this.dataValue)
        },
              ...
              if(backgroundArr.ColorType == 'SolidColor'){
                ...
                for(var j=0;j<2;j++){
                  var valueColor = {
                      offset:j*100 + '%',
                      color:backColor,
                      name:this.textblockData[i].Name
                  }

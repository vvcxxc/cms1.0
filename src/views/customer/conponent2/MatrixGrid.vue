              ...
              <template v-for="(cell, cellIndex) in data">
                ...
                <div class="matrixgrid-cell" :key="cellIndex" v-else 
                  ...
                  <div class="matrixgrid-set">
                    ...
                    <span class="add" @click="add(cell, item, $event)"></span>
                    <span class="subtract" @click="subtract(cell, item, $event)"></span>
                  </div>
                </div>
              </template>
            </div>
    ...
    <div v-show="TextBoxShow" style="width: 100%; height: 100%; position: fixed; z-index: 997">
      <div v-if="TextBoxShow" class="TextBoxPop_outPop">
        <div class="TextBoxPop_outHead">
          ...
          <span class="text">{{lang.MessageBox_Confrim}}</span>
        </div>
    ...
    arrFun() {
      ...
      for (var i = 0; i < this.data.Data.MatrixGridAnimationList.length; i++) {
        ...
        if (a != -1) {
          ...
          this.textBType.push(
            this.data.Data.MatrixGridAnimationList[i].TagDataType
          );
        }
      }
        ...
        matrixgridList.forEach((gridItem, idx)=>{
          if(gridItem.HasMatrixCellStyle && gridItem.ElementName === type){
            for(let i=0; i < gridItem.MatrixCellStyles.length; i++){
              ...
              //还原默认颜色
              let cellItem = this.dataValue[idx]
              item.Background = cellItem.Background
              item.fontColor = cellItem.fontColor
              item.textFillColor = cellItem.textFillColor
              item.backgroundClip = cellItem.backgroundClip
              item.isChange = false
            }
          }
        })
          ...
          for(let j=0;j<MatrixGridAnimationList.length; j++){
            if(MatrixGridAnimationList[j].ElementName === this.textblockData[i].Name){
              ...
              let tableStyle = {
                ...
                TagName: MatrixGridAnimationList[j].TagName,
                TagDataType: MatrixGridAnimationList[j].TagDataType,
                ...
                matrixGridElementName: MatrixGridAnimationList[j].MatrixGridElementName,

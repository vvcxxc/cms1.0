 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-02-05 18:41:03
 -->
<template>
    <div>
      ...
      <div class="NumberScreen30a" v-for="(item,index) in dataValue" :key="index">
        <div v-if="show" :class="'aa'+item.class"  
            ...
            + ';overflow:hidden;zIndex:'+item.ZIndex+';background:'+item.Background + ';transform:rotate(' + item.rotate + 'deg);boxShadow:'+item.Shadow">
                ...
                <el-select  class="elSelect" @focus="selectClick($event,item)" @change="selectFuna(item)" :disabled="item.disabled" 
        ...
        +item.TextDecorations+';color:'+item.Foreground+';font-size:'+item.size+'px'" />
        ...
        +'px;height:'+item.height + 'px;box-sizing: border-box;position:relative'">
         ...
         <input :disabled="item.disabled" class="elInput2"
         v-if="item.show" @keyup.enter="keyFun2(item)"  @mousedown="downClick(item,$event,'input1')" 
         ...
         +';text-decoration:'+item.TextDecorations+';color:'+item.Foreground+';font-size:'+item.size+'px'"
  ...
  methods: {
    ...
    selectFuna(item){
       if(item.value == '区间'){
         ...
         item.Inpwidth = 85+15
          setTimeout(()=>{
           var dom = document.querySelector(`.aa${item.class} .elInput1`).style.borderImage
           var width = document.querySelector(`.aa${item.class} .elInput1`).style.borderWidth
           var color = document.querySelector(`.aa${item.class} .elInput1`).style.color
           var size  = document.querySelector(`.aa${item.class} .elInput1`).style.fontSize
           var textDecoration  = document.querySelector(`.aa${item.class} .elInput1`).style.textDecoration
           var fontWeight  = document.querySelector(`.aa${item.class} .elInput1`).style.fontWeight
           document.querySelector(`.aa${item.class} .elInput2`).style.borderImage = dom
           document.querySelector(`.aa${item.class} .elInput2`).style.borderWidth = width
           document.querySelector(`.aa${item.class} .elInput2`).style.fontSize = size
           document.querySelector(`.aa${item.class} .elInput2`).style.fontWeight = fontWeight
           document.querySelector(`.aa${item.class} .elInput2`).style.color = color
           document.querySelector(`.aa${item.class} .elInput2`).style.textDecoration = textDecoration
         },200)
       }else{
         item.show =false
         item.Inpwidth = 0
             ...
             if(PressButtonList[j].ElementName == this.textblockData[i].Name){   //有变量
               ...
               var value = {                            //初始化样式赋值
                  ...
                  this.dataValue.push(value)
                   ...
                   //下拉框改变样式
                   setTimeout(()=>{
                      let dom = document.querySelector(`.aa${this.textblockData[i].Name} input`)

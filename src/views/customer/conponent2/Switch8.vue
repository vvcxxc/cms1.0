 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-02-22 16:10:11
 -->
<template>
<div>
                ...
                <div v-if="item.ModeType==2?true:false"  :name="item.ModeType" class="switch_Box" 
                    ...
                    <div v-if="item.FormType==6?false:true"  data="开关" :data1="item.textBl" class="switch_dom" :style="'width:' + item.wBlValue 
                        ...
                        <div class="switch_dom1" :style="'overflow:hidden;width:100%;height:100%;position:relative;lineHeight:' 
                            ...
                            <input style="pointer-events: none;" :data-form="item.FormType" :data-mode="item.ModeType" data="开关" :class="'aa' + item.class + 'ipt'"
                    ...
                    <div v-if="item.FormType==6?true:false" data="其他" class="switch_dom" :style="'width:' + item.wBlValue+ 'px; height:'
                        ...
                        <div class="switch_dom1" :style="'width:100%;height:100%;position:relative;lineHeight:' 
                            + item.hBlValue + 'px;boxShadow:' + item.Shadow">
                            <input style="pointer-events: none;" :data-form="item.FormType" :data-mode="item.ModeType" data="其他" :class="'aa' + item.class + 'ipt'"
                ...
                <div v-if="item.ModeType==1?true:false" data="原位"  :name="item.ModeType" class="switch_Box" 
                    ...
                    <div class="switch_dom" :style="'overflow:hidden;width:' + (item.width-2) + 'px; height:'
                        ...
                        + (item.height-2) + 'px;boxShadow:' + item.Shadow">
                            <input style="pointer-events: none;" :data-form="item.FormType" :data-mode="item.ModeType" data="原位" :class="'aa' + item.class + 'ipt'"
     ...
     this.jurisdictionShow(item).then(val => { 
         if(item.TagName != ''){ 
             ...
             //没有变量走接口默认开关
                ...
                if(iptCheck1 == true){
                    document.querySelector(`.aa${item.class}ipt`).checked = false
                    document.querySelector(`.aa${item.class}ipt`).nextElementSibling.nextElementSibling.innerHTML = item.OffText
                    $(`.aa${item.class}`).find('#switch-area')[0].style.background= "rgb(221, 221, 221)"
                }else{
                    document.querySelector(`.aa${item.class}ipt`).checked = true
                    document.querySelector(`.aa${item.class}ipt`).nextElementSibling.nextElementSibling.innerHTML = item.OnText
                    $(`.aa${item.class}`).find('#switch-area')[0].style.background= "rgb(221, 221, 221)"
                }
         }
     })

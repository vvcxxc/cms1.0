 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-02-01 22:02:16
 -->
<template>
<div>
      <div v-for="(item,index) in dataValue" :key="index">
        ...
        name="switch" :class="'aa' + item.class" class="Switch8a switch-btn" 
        :style="'position:absolute; left:' + item.left + 'px; top:' + item.top + 'px; width:' 
        + item.width + 'px; height:' + item.height + 'px; opacity:' + item.opacity+';zIndex:'+item.ZIndex">
                ...
                <div v-if="item.ModeType==2?true:false"  :name="item.ModeType" class="switch_Box" 
                    ...
                    <div v-if="item.FormType==6?false:true"  data="开关" :data1="item.textBl" class="switch_dom" :style="'width:' + item.wBlValue 
                        + 'px; height:' +item.hBlValue + 'px; position:absolute;top:0px;left:0px;bottom:0;border:1px solid #707070;border-radius:' 
                        + item.CornerRadius * 2 + 'px'">
                        <div class="switch_dom1" :style="'overflow:hidden;width:100%;height:100%;position:relative;lineHeight:' 
                            ...
                            <input style="pointer-events: none;" :data-form="item.FormType" :data-mode="item.ModeType" :class="'aa'+item.class + 'ipt'"
                            ...
                            + item.CornerRadius * 2 + 'px'"></span>

                            <span class="switch-text" :style ="'width:'+(100-40)+'%;fontSize:' + item.fontSize + 'px;fontFamily:' 
                    ...
                    <div v-if="item.FormType==6?true:false" data="其他" class="switch_dom" :style="'width:' + item.wBlValue+ 'px; height:'
                        + item.hBlValue + 'px; position:absolute;top:0;left:0;bottom:0;'">
                ...
                <div v-if="item.ModeType==1?true:false" data="原位"  :name="item.ModeType" class="switch_Box" 
                    ...
                    <div class="switch_dom" :style="'overflow:hidden;width:' + (item.width-2) + 'px; height:'
                        + (item.height-2) + 'px; position:absolute;top:0;left:0;bottom:0;border-radius:' + item.CornerRadius * 2 + 'px;border:'
                        ...
                        + (item.height-2) + 'px;boxShadow:' + item.Shadow">
                            ...
                            + (item.FormType ==5 ?'#00000000':item.OffColor) + '; border-radius:' + (item.CornerRadius * 2) + 'px'"></span>
    ...
    //加入变量   主页面也有加入变量
      ...
      judgeFun(data){
            for(let f=0;f<this.Starr.length;f++){
                    var iptDom = document.querySelector(`.aa${this.Starr[f]}ipt`)
  ...
  //确认
    ...
    switchFun(e,item){
     this.jurisdictionShow(item).then(val => { 
         if(item.TagName != ''){  //走接口等接口返回接口才改开关
             if(!this.CanExcuteShow){
                    var valueShow
                    var arr = []
                    var iptDom = document.querySelector(`.aa${item.class}ipt`)
                    ...
                    data: arr
                     ...
                     if(iptCheck == true){
                         ...
                         document.querySelector(`.aa${item.class}ipt`).checked = true
                     }
                    this.commerPopShow1 = true
             }
         }else{
             //没有变量走接口默认开关
                var iptCheck1 = document.querySelector(`.aa${item.class}ipt`).checked
                if(iptCheck1 == true){
                    document.querySelector(`.aa${item.class}ipt`).checked = false
                    document.querySelector(`.aa${item.class}ipt`).nextElementSibling.nextElementSibling.innerHTML = 'Off'
                    $(`.aa${item.class}`).find('#switch-area')[0].style.background= "rgb(221, 221, 221)"
                }else{
                    document.querySelector(`.aa${item.class}ipt`).checked = true
                    document.querySelector(`.aa${item.class}ipt`).nextElementSibling.nextElementSibling.innerHTML = 'On'
                    $(`.aa${item.class}`).find('#switch-area')[0].style.background= "rgb(221, 221, 221)"
                }
         }
     })
                               ...
                               //阴影
                                ...
                                if(borderSadow.OuterShadow != undefined){
                                  ...
                                  var value = {
                                      ...
                                      fontSize:Number(this.textblockData[i].PropertyList.FontSize),
                                      class:this.textblockData[i].Name,
                                      OnColor:'#'+switchlist[j].OnColor.slice(3) + switchlist[j].OnColor.slice(1,3),

 ...
 * @Author: Tao
 * @LastEditors: Tao
 * @LastEditTime: 2021-02-22 13:47:47
 -->
<template>
<div>
<div v-for="(item,index) in dataValue" :key="index">
  <div
      ...
      + 'deg); cursor:pointer;zIndex:'+item.ZIndex+'; background:' + item.BorderBrush + ';boxSizing:border-box;padding:'
      + item.BorderThickness + 'px;overflow:hidden;white-space:nowrap;boxShadow:'+item.Shadow">
...
<div v-show="commerPopShow1" style="width:100%;height:100%;position:absolute;z-index:2147483647">
  <div v-if="commerPopShow1" class="commerPop_outPop">
  <div class="commerPop_outHead">
      <i class="warning el-icon-warning"></i>
      <span class="text">提示</span>
  </div>
  <div class="commerPop_conter">{{tipsText}}</div>
              ...
              if(!this.CanExcuteShow){
                ...
                for(let i=0;i<tabname.length;i++){
                  if(item.class == tabname[i].ElementName){
                    ...
                    var SCMSModelID = tabname[i].SCMSModelID
                    var tabId = 'AA239AE2-8B58-4404-A275-C0EC57160AA5'
                    if(SCMSModelID.toUpperCase() != tabId.toUpperCase()){
                      this.$parent.tabPop()
                        return
                    }
                            ...
                            for(var u=0;u<menuArr.length;u++){
                              ...
                              if(menuArr[u].id == res.data.data){
                                ...
                                //  this.$router.push({path:menuArr[u].to,query: {id:menuArr[u].id}});
                                 this.$parent.tabJson(menuArr[u].to,menuArr[u].id)
                              }
                              if(menuArr[u].children != null){
                                for(var y=0;y<menuArr[u].children.length;y++){
                                  if(menuArr[u].children[y].id == res.data.data){
                                    ...
                                    // this.$router.push(`${menuArr[u].children[y].to}`);
                                    this.$parent.tabJson(menuArr[u].children[y].to,menuArr[u].children[y].id)
                                  }
                                }
                              }

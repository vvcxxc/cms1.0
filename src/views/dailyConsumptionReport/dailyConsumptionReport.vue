<template>
    <div class="DailyConsumptionReport-Page" :class="{ blackBlueBg: $store.state.color === 'blackBlue' }">
...
export default {
    ...
    methods: {
        initTable(query) {
            this.$axios({
                method: 'get',
                url: `/api/ReportManagement/ConsumeTrendDayReportQuery?consumptionType=${query.paramType}&time=${query.time}&isBSystem=${query.isBSystem}`,
            }).then((res) => {
                if (res.data.code == 0) {
        ...
        formSave() {
            ...
            this.$axios({
                method: 'post',
                url: `/api/ReportManagement/EditConsumeTrendDayReport?consumptionType=${this.query.paramType}&time=${this.query.time}`,
                data: queryList,
            }).then((res) => {
                this.tipShow = true;
                this.tipword = '保存成功！';

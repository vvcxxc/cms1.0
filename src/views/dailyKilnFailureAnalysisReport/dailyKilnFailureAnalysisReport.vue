<template>
    <div class="DailyKilnFailureAnalysisReport-Page" :class="{ blackBlueBg: $store.state.color === 'blackBlue' }">
        <div class="page-center">
            ...
            <div class="content flex">
                <div class="content-left">
                    <my-form :list="list" :flag="flag" @handleChange="handleChange" />
                </div>
...
export default {
    ...
    methods: {
        initTable(query) {
            this.$axios({
                method: 'get',
                url: `/api/ReportManagement/KilnFaultAnalysisDayReportQuery?time=${query.time}&isBSystem=${query.isBSystem}`,
            }).then((res) => {
                if (res.data.code == 0) {
        ...
        formSave() {
            ...
            this.$axios({
                method: 'post',
                url: `/api/ReportManagement/EditKilnFaultAnalysisDayReport?time=${this.query.time}`,
                data: queryList,
            }).then((res) => {
                this.tipShow = true;
                this.tipword = '保存成功！';
        ...
        exportTable() {
            require.ensure([], () => {
                ...
                this.json2excel(excelDatas, '窑炉故障日分析报表', true, 'xlsx');
            });
        },
...
.DailyKilnFailureAnalysisReport-Page {

<template>
    <div
        class="DailyKilnFailureAnalysisReport-Page"
...
export default {
    ...
    methods: {
        initTable(query) {
            this.$axios({
                method: 'get',
                url: `/api/KilnFailure/GetDailyFailureDataByTime?time=${query.time}&isBSystem=${query.isBSystem}`,
            }).then((res) => {
        ...
        formSave() {
            ...
            this.$axios({
                method: 'post',
                url: `/api/KilnFailure/EditDailyFailureData?time=${this.query.time}`,
                data: queryList,
            }).then((res) => {
                this.tipShow = true;
                this.tipword = '保存成功！';

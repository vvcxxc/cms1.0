...
export default {
    ...
    computed: {
        ...
        broCompData: function () {
            const node = this.materialParent.$refs[broComp];
            console.log(node, ' -=------');
    ...
    watch: {
        ...
        'broCompData.currentRowData.Id': {
            handler: function (val, oldVal) {
                console.log(val, 'oldVal currentRowData');
    ...
    data() {
        return {
            ...
            managementHeader: [
                { prop: 'BarcodeCheckItem', label: '物料校验段' },
                { prop: 'BeginIndex', label: '校验起始位数' },
                { prop: 'CodeLength', label: '校验段长度' },
            ],
            managementBtnList: [
                {
                    label: '添加',
    ...
    methods: {
        ...
        async onDialogConfirm() {
            ...
            if (BTN_EMIT.ADD === this.currentBtnEmitType) {
                ...
                interResult = await api.updateManageCode(
                    this.tableSelect.Id,
                    formData
                );
            }
            ...
            const msg = codeStatus
                ? '保存成功'
                : _.get(interResult, 'data.msg') || '保存失败';

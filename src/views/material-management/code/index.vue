<template>
    <div style="height: 100%">
        <div class="management">
            <div class="management-header">
                <span class="title" :style="{ fontSize: 20 * a11 + 'px' }">物料校验规则</span>
            ...
            <el-table :data="tableData" style="width: 100%; overflow: auto" @row-click="handleRowChange" stripe
    ...
    padding: '0',
        ...
        <CodeDialog v-if="dialogVisible" :appendToBody="true" :visible="dialogVisible" :title="dialogTitle"
            ref="managementCodeDialogRef" :dialogFormData="dialogFormData" :rulesTypeList="rulesTypeList"
            @close="dialogVisible = false" @confirm="onDialogConfirm" :currentRowData="broCompData.currentRowData" />
    </div>
</template>
    ...
    methods: {
        getPower() {
            ...
            this.buttonarr.forEach(item => {
                if (item.RightName == '物料管理-物料校验添加按钮') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '物料管理-物料校验编辑按钮') {
                    this.bjid = item.RightID;
                } else if (item.RightName == '物料管理-物料校验删除按钮') {
        ...
        async onActionSort(row, direction) {
            ...
            const msg = code === 0 ? _.get(data, 'data.data') : _.get(data, 'data.msg')

<template>
    <div style="height: 100%">
        <div class="management">
            <div class="management-header">
                <span class="title" :style="{ fontSize: 20 * a11 + 'px' }">物料管理</span>
    ...
    padding: '0',
        ...
        <ManagementDialog v-if="dialogVisible" :appendToBody="true" :visible="dialogVisible" :title="dialogTitle"
            ref="managementDialogRef" :dialogFormData="dialogFormData" :deviceList="deviceList"
            @close="dialogVisible = false" @confirm="onDialogConfirm" />
    </div>
</template>
    ...
    methods: {
        getPower() {
            ...
            this.buttonarr.forEach(item => {
                if (item.RightName == '物料管理-物料添加按钮') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '物料管理-物料编辑按钮') {
                    this.bjid = item.RightID;
                } else if (item.RightName == '物料管理-物料删除按钮') {
        ...
        async initData() {
            const data = await api.getManageList();
            this.tableData = _.get(data, 'data.data', []);

            this.$refs.singleTable.setCurrentRow(this.tableData[0]);
            this.currentRowData = _.head(this.tableData);
            console.log(this.tableData, 'create manage init');
        },

        async initDeviceList() {
            const data = await api.getEquipmentData();
            this.deviceList = _.get(data, 'data.data', []);
        },
        ...
        async onBtnClick(item) {
            ...
            if (BTN_EMIT.EDIT === keyword) {
                ...
                if (!this.currentRowData) {
                    this.changeTipPopConfig('请先选择对应物料信息！')
                    return;
                }
                const result = await api.deleteOrEditVerify({
                    argMIds: this.currentRowData.MId,
                    Del: false
                });
                const isUsed = _.get(result, 'data.code', 0) === 1 ? true : false
                if (isUsed) {
                    this.changeTipPopConfig(_.get(result, 'data.msg', "当前所选物料已被引用，无法编辑！"), true)
                    this.isPopShow = true
                    return
                }
                ...
                if (!this.currentRowData) {
                    ...
                    const isUsed = _.get(result, 'data.code', 0) === 1 ? true : false
                    if (isUsed) {
                        this.changeTipPopConfig(_.get(result, 'data.msg', "当前所选物料已被引用，无法删除！"), true)
                        this.isPopShow = true
                    } else {
                        this.changeTipPopConfig('是否确认删除', false)
                        this.isPopShow = true
                    }
                    return
                }
        ...
        async tipCallBack(str) {
            ...
            this.noCancel = true;
            if (str == 'yes') {
                const result = await api.delManage(this.currentRowData.MId);
        ...
        async onDialogConfirm() {
            ...
            if (BTN_EMIT.ADD === this.currentBtnEmitType) {
                interResult = await api.addManage(formData);
            } else if (BTN_EMIT.EDIT === this.currentBtnEmitType) {
                interResult = await api.updateManage(formData);
            }

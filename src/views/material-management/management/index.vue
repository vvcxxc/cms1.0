    ...
    methods: {
        getPower() {
            ...
            this.buttonarr.forEach(item => {
                if (item.RightName == '物料管理-添加物料') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '物料管理-编辑物料') {
                    this.bjid = item.RightID;
                } else if (item.RightName == '物料管理-删除物料') {
        ...
        async onBtnClick(item) {
            ...
            if (BTN_EMIT.EDIT === keyword) {
                ...
                const isUsed =
                    _.get(result, 'data.code', 0) === 1 ? true : false;
                if (isUsed) {

<template>
    <div class="MonthlyKilnFailureAnalysisReport-Page" :class="{ blackBlueBg: $store.state.color === 'blackBlue' }">
...
export default {
    ...
    methods: {
        initTable(query) {
            this.$axios({
                method: 'get',
                url: `/api/ReportManagement/KilnFaultAnalysisMonthReportQuery?time=${query.time}&isBSystem=${query.isBSystem}`,
            }).then((res) => {
                if (res.data.code == 0) {
        ...
        formSave() {
            ...
            this.$axios({
                method: 'post',
                url: `/api/ReportManagement/EditKilnFaultAnalysisMonthReport?time=${this.query.time}`,
                data: queryList,
            }).then((res) => {
                this.tipShow = true;
                this.tipword = '保存成功！';

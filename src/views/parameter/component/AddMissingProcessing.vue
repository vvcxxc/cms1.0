<template>
    <div class="EditWorkStepsPop">
                ...
                <div class="content1">
                    <div class="content1-line line-bottom">
                        <div class="content2">
                            ...
                            <el-select @change="getConfig" v-model="data.FormulaId" placeholder="必选"
                    ...
                    <div class="content1-line">
                        ...
                        <div class="content2">
                            ...
                            <el-input type="text" v-model="data.BarCodeType" style="width: calc(100% - 100px)"
                                placeholder="必填" :disabled="true" />
                            <div class="unit" @click="handleChoose('BarCodeType')">…</div> -->
                        </div>
                    </div>
        ...
        <ChooseProcess v-if="ChooseProcessShow" @callback="ChooseProcessCallback" :selectItem="data.Items" :WID="WID" />
    </div>
</template>
...
export default {
            ...
            .then(res => {
                if (res.data.code == 0) {
                    ...
                    if (this.ProductTypeList.length) {
                        if (this.ProductTypeList.some((item) => item.IsApp)) {
                            this.data.FormulaId = this.ProductTypeList.find((item) => item.IsApp).FID;
                        } else {
                            this.data.FormulaId = this.ProductTypeList[0].FID;
                        }
    ...
    methods: {
        getConfig() {
            ...
            this.$axios({
                url: `/api/ProcessParameterConfigure/QueryMissedProcessesConfig?SID=${this.SID}&FID=${this.data.FormulaId}`,
                method: 'post',
            }).then((res) => {
                if (res.data.code == 0) {
        ...
        handleClick(str) {
            if (str == 'yes') {
                this.$axios({
                    ...
                    data: {
                        ...this.data,
                        Items: this.data.Items.map(_ => ({ WIDs: _.WID })),
                        SID: this.SID
                    }
                }).then((res) => {
                    if (res.data.code == 0) {

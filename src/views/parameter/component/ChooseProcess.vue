...
export default {
    ...
    methods: {
        getData() {
            this.$axios({
                url: `/api/ProcessParameterConfigure/QueryWorkSectionsByFormula?FID=${this.FID}&KeyWrod=${this.filterStr}`,
                method: 'post',
            }).then((res) => {
                if (res.data.code == 0) {
                    ...
                    if (this.WID) {
                        this.dataList = res.data.data.filter(_ => _.WID != this.WID)
                    } else {
                        this.dataList = res.data.data
                    }

                    console.log(this.selectItem, 'selectItem')
                    if (this.selectItem && this.selectItem.WorkCode) {
                        let _selectList = this.selectItem.WorkCode.split(',');

                        if (_selectList && _selectList.length) {
                            this.checkList = this.dataList.filter(_ => _selectList.some(_2 => _.WorkCode == _2))
                            this.$nextTick(() => {

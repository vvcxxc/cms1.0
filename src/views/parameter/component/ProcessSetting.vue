        ...
        <div class="myDialog" id="addDialog">
            ...
            <div class="popContent">
                ...
                <div class="content1-line">
                    <el-radio-group class="ml-4" v-model="RefixDataHandleTypeValue">
                        <el-radio :label="0" size="large">仅展示最新加工数据（重复产品码旧数据删除）</el-radio>
                        <el-radio :label="1" size="large">展示所有数据（重复产品码旧数据保留）</el-radio>
                    </el-radio-group>
                </div>
                ...
                <div class="content1-line">
                    <div class="content">
                        时间范围：最近
                        <el-input style="width:100px;margin:0 10px;" v-model="MissDay" />天
                    </div>
                </div>
...
export default {
    ...
    mounted() {
        this.$axios({
            ...
            if (res.data.code == 0) {
                ...
                this.RefixDataHandleTypeValue = res.data.data.RefixDataHandleTypeValue;
            } else {
    ...
    methods: {
        ...
        handleClick(str) {
            if (str == 'yes') {
                this.$axios({
                    url: `/api/ProcessParameterConfigure/SetGloabl?Day=${this.MissDay}&refixDataHandle=${this.RefixDataHandleTypeValue}`,
                    method: 'post',
                }).then((res) => {
                    if (res.data.code == 0) {
                        if (this.title == '添加工步') {

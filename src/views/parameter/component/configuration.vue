    ...
    <div v-loading="loading || loading2 || loading3" ref="head" class="tapwater" :style="{ zoom: zoomValue }">
            ...
            <div class="query-table clearfix">
                <div class="fl">
                    ...
                    <el-select style="margin-left:5px;width:120px" v-model="ProductTypeCode" @focus="getZoom()"
                        :placeholder="lang.SCMSConsoleWebApiMySql_PleChoose" @change="allConfigure(false)">
                    ...
                    <div class="edit" @click="openProductScrapping">产品报废</div>
                                ...
                                <table cellspacing="0" cellpadding="1">
                                    <thead>
                                        ...
                                        <tr>
                                            ...
                                            <th colspan="1" rowspan="2" class="blue-th no3">操作员</th>
                                            <th colspan="1" rowspan="2" class="blue-th no3">订单号</th>
                                            <th colspan="1" rowspan="2" class="blue-th no3">产品型号</th>
                                            <th colspan="1" rowspan="2" class="blue-th no3">模式</th>
                                            ...
                                            <th colspan="1" rowspan="2" class="blue-th no6">进站时间</th>
                                    ...
                                    <tbody>
                                        <tr v-for="item in tableData.dataList" :key="item.Number + 'data'">
                                            ...
                                            <td class="no6">{{ item.EntryTime }}</td>
                                            ...
                                            <template v-for="(item2, _idx) in tableData.curLeft.WorkSteps">
                                                <td colspan="1" rowspan="1" v-for="item3 in item2.Projects "
                                                    ...
                                                    <div class="chartBtn"
                                                        v-if="item3.IsConvertImages || item3.ParameterTypeValue == 2"
                                                        @click="openChart(item3, item)">
        ...
        <div class="chartPop" v-if="chartShow">
            <div class="myDialog">
                <div class="popHeader">
                    {{ chartName }}
                    <img class="popHeaderimg" :src="no" alt="" @click="closeChart('no')" />
                </div>

                <div class="popContent" id="itemChart">
                </div>
                <div class="popBtn">
                    <el-button class="pop-sumbit" @click="closeChart('no')"
    ...
    methods: {
        openChart(item3, item) {
            ...
            this.$nextTick(() => {
                ...
                if (item3.ParameterTypeValue == 2) {
                    this.$axios({
                        method: 'post',
                        url: `/api/ProcessParameterReport/GetSafetyRegulationRoomDockParams?ProductId=${item.ProductID}&ProjectId=${item3.PID}`,
                    }).then(res => {
                        if (res.data.code == 0) {
                            let option = {
                                legend: {
                                    right: '4%',
                                    data: ['上限值', item3.ProjectName, '下限值']
                                },
                                tooltip: {
                                    trigger: 'axis'
                                },
                                ...
                                series: [
                                    {
                                        ...
                                        type: 'line',
                                        symbol: 'none',
                                        data: res.data.data.xAxis.map(_ => res.data.data.upperLimit),
                                        lineStyle: {
                                            normal: {
                                                color: '#d9001b',
                                                width: 1,
                                    ...
                                    },
                                    {
                                        ...
                                        type: 'line',
                                        symbol: 'none',
                                        data: res.data.data.xAxis.map(_ => res.data.data.lowerLimit),
                                        lineStyle: {
                                            normal: {
                                                color: '#f59a23',
                                                width: 1,
                    ...
                    this.$axios({
                        method: 'post',
                        url: `/api/ProcessParameterReport/GetdataRefrigerationSystemDockParams?ProjectId=${item3.PID}&ProductId=${item.ProductID}`,
                    }).then(res => {
                        if (res.data.code == 0) {
                            let option = {
                                tooltip: {
        ...
        powerBtn() {
            ...
            this.scrappingId = btnObj['参数报表-产品报废按钮'].RightID
        ...
        allConfigure(isfirst) {
            this.$axios({
                method: 'post',
                url: `/api/ProcessParameterReport/QueryWorkSections?ProductTypeCode=${this.ProductTypeCode}`,
            }).then(res => {
        ...
        tableScroll() {
            ...
            _dom.addEventListener('scroll', () => {
                ...
                if (delta > 0 && (scrollTop + windowHeight >= scrollHeight)) {
                    ...
                    setTimeout(() => { this.loadTimer = false }, 1000)
                }
            })
        },
        changeLeft(curLeft) {
        ...
        exportXlsxFun() {
            ...
            window.open(
                `/api/ProcessParameterReport/Export?wid=${this.tableData.curLeft.WID}&QualitiedType=${this.QualitiedType}&QueryKeyType=${this.QueryKeyType}&startTime=${this.timevalue1}&endTime=${this.timevalue2}&code=${this.codeText}&accurate=${accurate}&argLanguage=${localStorage.getItem('currentLang')}`
            );
        },
        //导出图片
        exportPngFun() {

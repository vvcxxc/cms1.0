...
<template>
    <div class="container">
        ...
        <div class="tip" ref="kongtiao8" v-show="tipchange">
            <div
                class="tiphead"
        ...
        <div class="table" :class="{ colortip: $store.state.color == 'grey' }">
            ...
            <div class="second">
                <div class="firsthead" :style="{ fontSize: 20 * a11 + 'px' }">
                    ...
                    <div class="missBtn" @click="openMissingProcessing">
                        漏工序检测
                    </div>
                </div>
                <div class="firsttable">
        ...
        <div class="onework" v-show="onework">
            ...
            <div class="oneworkname">
                <span> <span class="require">*</span>工序名称</span>
                <el-input type="text" v-model="oneObj" style="width:310px" />
            </div>

            <div class="oneworkname2">
                <span> <span class="require">*</span>工序属性</span>
                <el-select
                    type="text"
                    v-model="WorkType"
                    style="width:310px"
                    :disabled="onechange != 2"
                    @change="locatorConfigFn"
                >
                    <el-option
            ...
            <div class="oneworkname3" v-if="locatorConfigDtos.length">
                <div class="table-label" :style="{ fontSize: 20 * a11 + 'px' }">
                    货位信息
                </div>
                <div class="table-box">
                    <el-table
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope">
                                <el-input v-model="scope.row.LocatorId" />
                            </template>
                        </el-table-column>
                        <el-table-column
                            prop="LocatorStatusVarName"
                            label="货位状态"
                            :show-overflow-tooltip="true"
                        >
                            <template slot-scope="scope">
                                <div class="table-unitContent">
                                    {{ scope.row.LocatorStatusVarName }}
                                    <div
                                        class="unit"
                                        @click="
                                            handleChoose(
                                                scope,
                                                'LocatorStatusVarName'
                                            )
                                        "
                                    >
                                        …
                        ...
                        <el-table-column
                            prop="OfflineQtyVarName"
                            label="下线数量"
                            :show-overflow-tooltip="true"
                        >
                            <template slot-scope="scope">
                                <div class="table-unitContent">
                                    {{ scope.row.OfflineQtyVarName }}
                                    <div
                                        class="unit"
                                        @click="
                                            handleChoose(
                        ...
                        <el-table-column
                            prop="CallMaterialResultVarName"
                            label="叫料校验结果"
                            :show-overflow-tooltip="true"
                        >
                            <template slot-scope="scope">
                                <div class="table-unitContent">
                                    {{ scope.row.CallMaterialResultVarName }}
                                    <div
                                        class="unit"
                                        @click="
                                            handleChoose(
        ...
        >
            ...
            <div class="lookcontent" :style="{ paddingTop: 30 * a11 + 'px' }">
                ...
                <div class="cancel" @click="cancel1">
                    {{ lang.PopupCommon_Cancel }}
                </div>
                <div class="pre" @click="blpre">
                    {{ lang.PopupCommon_Save }}
                </div>
            </div>
        </div>
        <div class="Unqualified" ref="kongtiao4" v-show="unsixth">
            <div
        ...
        <div class="newblue" ref="kongtiao5" v-show="newblue">
            <div
                class="newbluehead"
                :class="{ colordiv: $store.state.color == 'grey' }"
            >
        ...
        <div class="changpro" ref="kongtiao6" v-show="projectchange">
            <div class="changehead">
                <div class="changetop"></div>
                <span>{{
                    lang.ProcessParameterConfigure_ModifyWorkProject1_ModificationItem
                }}</span>
                <img
                    :src="no2"
                    alt
            ...
            <div class="changecontent">
                ...
                <div class="changestion">
                    ...
                    <input
                        type="number"
                        v-model="threeObj.Zoom"
                        placeholder="请输入数字"
                    />
                </div>
                <div class="changestion">
                    <span> </span>
                    <el-radio-group v-model="threeObj.IsBeforeWeight">
                        <el-radio :label="true">滴漆前重量</el-radio>
                        <el-radio :label="false">滴漆后重量</el-radio>
                    </el-radio-group>
                </div>
                <div class="changestion footer">
                    <div class="cancel" @click="cancel">
                        {{ lang.PopupCommon_Cancel }}
                    </div>
        ...
        >
            ...
            <div class="pre" @click="pre11">{{ lang.PopupCommon_Save }}</div>
        </div>
        ...
        <AddMissingProcessing
            v-if="missingProcessingShow"
            @callback="missingProcessingCallback"
            :SID="workproject"
        />
        <AddWorkStation
            v-if="twowork"
            :title="editWorkStationType"
            @callback="gxpre2"
            :WID="oneworkduan"
            :selectItem="twoObj"
        />
...
export default {
    ...
    methods: {
        ...
        variableSelectCallBack(str, val) {
            ...
            console.log(str, val);
            if (str == 'yes') {
                this.locatorConfigDtos[this.chooseRow][this.chooseText] =
                    val.Name;
                console.log(this.locatorConfigDtos);
            }
        },
        tipCallBack(str) {
            if (str == 'yes') {
        ...
        onepre() {
            ...
            if (this.onechange == 2) {
                ...
                this.$axios({
                    method: 'post',
                    url: `/api/ProcessParameterConfigure/AddWorkSection?argWId=${''}&argName=${
                        this.oneObj
                    }&workType=${this.WorkType}&LoginName=${_name}`,
                    data: this.locatorConfigDtos
                }).then(res => {
                    this.pdyd5 = true;
                    setTimeout(() => {
                        $('.tip').css({
                ...
                this.$axios({
                    method: 'post',
                    url: `/api/ProcessParameterConfigure/AddWorkSection?argWId=${this.oneworkduan}&argName=${this.oneObj}&workType=${this.WorkType}&LoginName=${_name}`,
                    data: this.locatorConfigDtos
                }).then(res => {
                    this.pdyd5 = true;
                    setTimeout(() => {
        ...
        changeone() {
            ...
            if (_obj) {
                ...
                this.WorkType = _obj.WorkType;
                this.locatorConfigDtos = JSON.parse(_obj.ExtraProperties) || [];
            } else {
                this.WorkCode = '';
                this.WorkType = 0;
                this.locatorConfigDtos = [];
            }
            // if (this.oneworkname !== '') {
            setTimeout(() => {
                this.onework = true;
        ...
        getWorkTypeList() {
            this.$axios({
                method: 'post',
                url: '/api/ProcessParameterConfigure/QueryWorkTypeList'
            }).then(res => {
                if (res.data.code == 0) {
                    this.WorkTypeList = res.data.data || [];
                    this.WorkTypeMap = this.WorkTypeList.reduce((acc, cur) => {

    ...
    <div class="page-full">
        <div class="page-contain">
            <div class="search-container">
                ...
                >
                    <template #btnGroup>
                        <button class="btn btn-success" @click="openVer(false)">
                            {{ lang.RoleManage_Add }}
                        </button>
                    ...
                    <template #default>
                        <el-upload
                            ...
                            :http-request="importPlan"
                        >
                            <button class="importBtn" @click="beforeimportPlan">
                                {{
                ...
                <div class="table-contain full-flex">
                    ...
                    >
                        <template #OptionList="{scope}">
                            <button
                                class="btn opt-btn Issued"
                                @click="issuedClick(scope)"
                                :disabled="!scope.DownloadEnabled"
                                title="下发"
                            ></button>
                            <!-- <button class="btn opt-btn undo" disabled></button> -->
                            <button
                                class="btn opt-btn pause"
                                @click="pauseClick(scope)"
                                :disabled="!scope.PauseEnabled"
                                title="暂停"
                            ></button>
                            <button
                                class="btn opt-btn stop"
                                @click="finishClick(scope)"
                                :disabled="!scope.FinishEnabled"
                                title="结束"
                            ></button>
                            <button
                                class="btn opt-btn edit"
                                @click="openVer(true, scope)"
                                title="修改"
                            ></button>
                        </template>
                    </my-table>
                    ...
                    <div class="online-item">
                        ...
                        <div class="item-value">
                            <el-input
                                v-model="editPlan.No"
                                :disabled="
                                    isEdit && editPlan.ShowActualStartTime
                                "
                            />
                        </div>
                    </div>
        ...
        <loginPop v-if="verShow" @callback="verBack" :isEdit="isEdit" />
    </div>
</template>
<script>
...
export default {
        ...
        return {
            ...
            tableHead: {
                ...
                StatusDisplay: {
                    name: '计划状态',
                    computed: (row, key, c) => {
                        return `<div class="status">                  <div class="status-circle"
                    style="  background: ${c.StatusColor}  "></div>
                  ${row}</div>`;
                    }
                },
                DrawingNo: '图号',
            ...
            actionEvent: {
                issued: () => {
                    this.$axios
                        .post('/api/ProductionManage/DownloadPlan', {
                ...
                pause: () => {
                    // 暂停
                    this.$axios
                        .post('/api/ProductionManage/PausePlan', {
                            Operator: this.getOperator('SCMSUserName'),
                            Id: this.action.row.Id
                        })
                        .then(res => {
                ...
                finish: () => {
                    this.$axios
                        .post('/api/ProductionManage/FinishPlan', {
                            Operator: this.getOperator('SCMSUserName'),
                            Id: this.action.row.Id
                        })
                        .then(res => {
    ...
    methods: {
        getPower() {
            ...
            console.log('666l', this.buttonarr);
            this.buttonarr.forEach(item => {
                if (item.RightName == '生产计划-删除计划') {
                    this.scid = item.RightID;
                } else if (item.RightName == '生产计划-添加计划') {
                    this.tjid = item.RightID;
                } else if (item.RightName == '生产计划-导入按钮') {
                    this.drid = item.RightID;
                } else if (item.RightName == '生产计划-结束计划') {
                    this.jsid = item.RightID;
                } else if (item.RightName == '生产计划-暂停计划') {
                    this.ztid = item.RightID;
                } else if (item.RightName == '生产计划-修改计划') {
                    this.xgid = item.RightID;
                } else if (item.RightName == '生产计划-下发计划') {
            ...
            this.$axios({
                method: 'post',
                url: `/api/UserManage/UserManage_CheckAuthority?argUserID=${userid}&argRightID=${this.ztid}`
            })
                .then(res => {
                    this.ztshow = res.data.data;
                })
                .catch(err => {
                    console.log('err', err);
                });
        ...
        getList(index = null) {
            ...
            this.$axios
                .get(
                    encodeURI(
                        `/api/ProductionManage/GetProductionPlans?status=${Number(
                            planStatus
                        )}&start=${argStartTime}&end=${argEndTime}&keyword=${classes}&pageSize=${PageSize}&pageIndex=${index ||
                            PageIndex}`
                    )
                )
                .then(res => {
        ...
        verifyNumber(value, obj, key) {
            obj[key] = (value + '').replace(/^(0+)|[^\d]+/g, '');
        },
        // 添加生产计划
        addPlan() {
        ...
        importPlan(file) {
            ...
            this.$axios({
                method: 'post',
                url: `/api/ProductionManage/ImportPlanFile`,
                data: formData
            }).then(res => {
                if (res.data.code === 0) {
                    this.tipchange = true;
        ...
        issuedClick(row) {
            ...
            console.log(row);
            this.action.type = 'issued';
            this.action.row = row;
            this.showBtnGroup = true;
            this.tipchange = true;
            this.tipword = '是否下发编号为' + row.No + '的生产计划？';
        },
        pauseClick(row) {
            if (!this.ztshow) {
                this.tipchange = true;
            ...
            this.action.type = 'pause';
            this.action.row = row;
            this.showBtnGroup = true;
            this.tipchange = true;
            this.tipword = '是否暂停编号为' + row.No + '的生产计划？';
        },
        finishClick(row) {
            if (!this.jsshow) {
                this.tipchange = true;
            ...
            this.action.type = 'finish';
            this.action.row = row;
            this.showBtnGroup = true;
            this.tipchange = true;
            this.tipword = '是否结束编号为' + row.No + '的生产计划？';
        },
        openVer(isEdit, scope) {
            if (!isEdit && !this.tjshow) {
                this.tipchange = true;
                this.tipword = '该用户没有操作权限';
                return;
            }
            if (isEdit && !this.xgshow) {
                this.tipchange = true;
                this.tipword = '该用户没有操作权限';
                return;
            }
        ...
        confirmMsg() {
            ...
            if (this.isEdit) {
                ...
                // 新建
                let createdPlan = JSON.parse(JSON.stringify(this.editPlan));
                Reflect.deleteProperty(createdPlan, 'Id');
                Reflect.deleteProperty(createdPlan, 'ActualStartTime');
                this.$axios
                    .post('/api/ProductionManage/AddPlan', createdPlan)
                    .then(res => {
                        if (res.data.code === 0) {
                            this.getList();
                            this.showMsg = false;
...
.opt-btn {
    ...
    &.Issued {
        ...
        &:disabled {
            &::after {
                background: url(~@/assets/images/icon_xiafa_dis.png) no-repeat;
                background-size: 100%;
            }
        }
    }

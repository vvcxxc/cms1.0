<!-- 动作分析 -->
<template>
  <div class="action-analy flex"  >
    <!-- 导航 -->
    <div class="tabs">
      <div v-for="(item, i) in menuList" :key="i" @click="linkPage(item.path)" class="itme flex"
...
export default {
  ...
  data() {
    return {
      ...
      menuList: [
        ...
        {
          name: '动作超时',
          path: '/ActionOversteps'
        },
        {
          name: '动作分布',
          path: '/ActionDistributions'
        },
      ],
      zoomValue: 0,
      activeName: 'second',
  ...
  methods: {
    ...
    initContainerSize() {
      ...
      var toolbarZ = $('.v-toolbar')[0].style.zoom || 1;
      var footerH = $('.v-footer').height();
      var footerZ = $('.v-footer')[0].style.zoom || 1;
      var marginTop = toolbarH * toolbarZ + 'px';
      let height = toolbarH * toolbarZ + footerH * footerZ + 'px';
      $('.action-analy').css({ marginTop, height: `calc(100% - ${height})` });
    },
    // 跳转页面
    linkPage(path) {

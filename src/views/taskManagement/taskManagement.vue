...
<template>
    ...
    left: 0;"></div>
            ...
            <RwEdit v-if="isxdEditShow" :data="xdSelectRow" @xdEditPopCallBack="xdEditPopCallBack" />
...
export default {
    ...
    created() {
        ...
        this.$axios({
            url: `/api/Option/GetMissionStatus`,
            method: 'get',
    ...
    methods: {
        ...
        exportTable() {
            ...
            this.$axios
                .get(
                    `/api/Mission/GetPagedList?startTime=${this.searchData.startTime}&endTime=${this.searchData.endTime}&filter=${this.searchData.argKeyword}&status=${this.searchData.status}&type=${this.searchData.type}&pageIndex=1&pageSize=${this.pageData.TotalCount}`
                )
                .then(res => {
                    let filterVal = [
                        'Number',
        ...
        req(pageIndex, s) {
            ...
            this.$axios
                .get(
                    `/api/Mission/GetPagedList?startTime=${this.searchData.startTime}&endTime=${this.searchData.endTime}&filter=${this.searchData.argKeyword}&status=${this.searchData.status}&type=${this.searchData.type}&pageIndex=${pageIndex}&pageSize=${this.pageData.PageSize}`
                )
                .then(res => {
                    if (res.data.code == 0) {

...
<template>
 <div  class="conterbox2" :class="{blackBlueBg: $store.state.color === 'blackBlue'}">
  ...
  <div class="conter_head" :class="{colortip:$store.state.color=='grey'}" :style="{zoom
		...
		<div class="bottom">
            <el-button @click.native="qeShow()" :disabled="showTime" class="conButton" type="primary">{{lang.NewTrendChart_NewTrendChartUserControl_Query}}</el-button>
		</div>
	</div>
	<div class="conter_echart" :style="{zoom,height:'427px',position:'relative'}">
      <div class="echarts echarttwo" :style="{width: 100/zoom+'%','background':color2,height:'427px',zoom:1/zoom,transform:'scale('+zoom+')'}" ref="tendency" :id='id'></div>
	</div>
...
export default {
 ...
 $('.bottom_fool').find('.el-input').css('zoom',this.zoom)
    ...
    // this.$nextTick(()=>{
    //       window.onresize = ()=> {
    //           console.log("asdasdasdsadasdasd",this.myChart)
...
for(let i=0;i<$('.el-picker-panel').length;i++){
 ...
 // 时间赋值
    ...
    drawLine(text){
        ...
        // if(!this.chartarr1.includes(this.myChart.id)){
        //           this.chartarr1.push(this.myChart.id)
        //         this.chartarr.push(this.myChart)
        // }
        // console.log(this.chartarr1)
        // console.log(this.$refs.tendency)
        //  let that = this
        // 	window.onresize = function(){
        //        that.a = 4
        //        console.log("触发123213213", that.a)
        //   }
        //   if(that.a!==0){
        //       debugger

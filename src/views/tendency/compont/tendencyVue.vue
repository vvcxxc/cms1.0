...
<template>
    <div class="conterbox container">
        ...
        <div class="conter_left" id="mymap" ref="mymap" :style="{ fontSize: 16 * zoom + 'px' }">
            ...
            <div v-if="!timeShow" class="QXtitle" style="position:absolute;left:38%;top:15px;z-index:99">{{ text }}
                查询时间{{ Sstime1 }}{{ Eetime1 }}</div>
            <div
                :style="{ background: color2, minHeight: '400px', height: 400 + 'px', width: '100%', zoom: zoom1, position: 'relative' }">
                <div :style="{ width: 100 / zoom1 + '%', height: '400px', zoom: 1 / zoom1, transform: 'scale(' + zoom1 + ')' }"
                    class="left_top" ref="tendency" id='qscxchartone'></div>
            </div>
            <div class="left_bottom">
                <div class="bottom_top" :style="{ zoom: zoom1 }" v-show='!RealShow'>
                    <el-tooltip class="item" popper-class="atooltip" effect="dark"
                        :content="lang.NewTrendChart_SingleChartUC_QueryForward1Hour" placement="top-start">
                        <div @mouseenter="atooltip" @click="arrowl('yl')" class="i_left el-icon-d-arrow-left"></div>
                    </el-tooltip>
                    <el-tooltip class="item" popper-class="atooltip" effect="dark"
                        :content="lang.NewTrendChart_SingleChartUC_QueryForward1Min" placement="top-start">
                    ...
                    <div v-if="!RealShow" style="font-size:14px" class="text">{{ Sstime1 }}
                        {{ lang.NewTrendChart_SingleChartUCViewModel_To }} {{ Eetime1 }}</div>
                    <el-tooltip class="item" popper-class="atooltip" effect="dark"
                        :content="lang.NewTrendChart_SingleChartUC_QueryBack1Hour" placement="top-start">
                        <div @mouseenter="atooltip" @click="arrow('yh')" class="i_right el-icon-d-arrow-right"></div>
                    </el-tooltip>
                    <el-tooltip class="item" popper-class="atooltip" effect="dark"
                        :content="lang.NewTrendChart_SingleChartUC_QueryBack1Min" placement="top-start">
                ...
                <div class="bottom_fool test" :style="{ background: '#fff !important' }" ref="table">
                    <el-table :porp="Newtime" border :row-style="{ height: 50 * zoom1 + 'px', fontSize: 14 * zoom1 + 'px' }"
                        :style="{ width: '100%', fontSize: 14 * zoom1 + 'px' }" height="100%" class="conterFool_middle"
                        ref="multipleTable" :data="curveArr" prop=Newtime tooltip-effect="dark" :header-cell-style="{
                        ...
                        <el-table-column :label="lang.NewTrendChart_SingleChartUC_No" prop="Number" :width="80 * zoom1">
                        </el-table-column>
                        <el-table-column prop="DisplayName" :label="lang.NewTrendChart_SingleChartUC_Name"
                            :width="150 * zoom1">
                        ...
                        <el-table-column :label="lang.NewTrendChart_SingleChartUC_Description" prop="Description"
                            :width="200 * zoom1">
                        </el-table-column>
                        <el-table-column :label="lang.NewTrendChart_SingleChartUC_Unit" prop="Unit" :width="150 * zoom1">
                        </el-table-column>
                        ...
                        <el-table-column :label="lang.NewTrendChart_SingleChartUC_Type" prop="Type" :width="150 * zoom1">
                        </el-table-column>

                        <el-table-column :min-width="180 * zoom1" :render-header="renderHeader">
                            <template slot-scope="scope">
            ...
            <div class="right_conter">
                <div class="conter_top">
                    ...
                    <div class="top_middle">
                        <div @click="curveName(index, 1, undefind, undefind, undefind, undefind, '表格点击')" class="top_conter"
                            v-for="(item, index) in curveID" :key="index">
                            ...
                            <div class="curvei">
                                ...
                                <div @click.stop="openClear(item, index)" class="delete_box">
                                    <i class="i_cache"></i>
                                </div>
                            </div>
                        </div>
        ...
        <!-- 编辑弹窗 -->
        <tenPop :Name="Namedata" :data="GIDdata" :type="PopType" class="tenpop" v-if="tenPopShow"
            :style="{ width: 1300 * zoom + 'px', height: 855 * zoom + 'px' }"></tenPop>
        <!-- <div ref="print" style="width: 720px;height: 500px;position:fixed;top:100px;left:0;">
                                                                <img :src="imgmap" style="width:100%;height:auto"/>
                                                            </div> -->
        <!-- 清除曲线 -->
        <clear-curve-pop :visible="clearShow" :grouId="grouId" :title="lang.NewTrendChart_ClearCurveSetting_Title"
            @close="clearCurveClose" @confirm="clearCurveConfirm" />
        <!-- 导出 -->
        <tenden-export :visible="exportShow" title="导出设置" @close="tendenExportClose" @confirm="tendenExportConfirm" />
    </div>
</template>
...
/* 2.2新增需求弹窗 */
import clearCurvePop from '../dialogs/clearCurve.vue'
import tendenExport from '../dialogs/tendenExport.vue'
...
export default {
    ...
    props: ['curveID', 'timeShow', 'Sstime', "Eetime", "qeShow", "Showtext", "WindowClick", "dataType"],
    data() {
        return {
    ...
    mounted() {
        ...
        if (!this.havecurveName && this.curveID && this.curveID.length) {
            this.curveName(0, 1)
        }
    },
    beforeDestroy() {
    ...
    methods: {
        /* 2.2新增需求 */
        openClear(item, index) {
            this.grouId = item.GID
            this.clearShow = true
        },
        clearCurveClose() {
        ...
        tendenExportConfirm(val) {
            ...
            if (val === 2) {
                ...
                // this.dataType true 实时  false 历史
                this.dataType
                this.Stime
                let postData = {}
                if (this.dataType) {
                    postData = {
                        ...
                        StartTime: this.Stime[0],
                        EndTime: this.Stime[this.Stime.length - 1],
                        Series: serDateArr
                    }
                } else {
                    postData = {
                ...
                this.$axios({
                    method: 'post',
                    url: `/api/NewTrendChart/Export`,
                    responseType: 'blob',
                    data: postData,
                }).then(res => {
                    if (res.status === 200) {
                        $this.downloadFile(res.data, '曲线.xlsx')
                    } else {
                        $this.confirm_Pop2($this, res.data.msg)
                    }
                })
            }
            this.tendenExportClose()
        },
        /* —————————————————————————————————————— */
        tip1() {
        ...
        timeaxiosAA(data, ind, data1) {
            ...
            clearInterval(this.time2);
            this.time2 = window.setInterval(() => {
                this.getDate(this.currentCache)
            }, this.refreshTime)
        ...
        curveName(index, indkk, data, ind, data1, text, txt) {
            this.havecurveName = true
            this.rowData = this.curveID[index]
            if (index == undefined) {
                index = 0
            }
            if (txt == '表格点击') {
        ...
        getDate() {
            ...
            if (this.indexTime == -1) {
                ...
                console.log('this.Stime', this.Stime, this.currentCache)
            }
        },
        ...
        drawLine(text) {
            ...
            let series = this.curveLineValue.map(_ => ({ ..._, data: _.data.slice(-this.Stime.length) }))
            // 绘制图表
            ...
            console.log(JSON.stringify(this.myChart.getOption()));
                    ...
                    .top_conter {
                        ...
                        .curvei {
                            ...
                            .i_cache {
                                ...
                                font-size: 20px;
                                color: #4ABFCD;
                                background: url('../../../assets/images/清除@2x.png');
                                background-size: 100% 100%;
                                margin-top: 3px;
                            }
                        }

<template>
    <div
        class="weightOfInjectionMachine-Page"
            ...
            <div class="content">
                <div class="table-content" id="table-content">
                    <el-table
                        ...
                        style="width: 100%"
                    >
                        <el-table-column prop="T1" label="" align="center">
                            <template slot-scope="scope">
                                <div
                                    v-if="
                                        scope.row.T1.text.indexOf('记录人') >= 0
                                    "
                                    class="left white"
                                >
                                    {{ scope.row.T1.text }}
                                </div>
                                <span v-else>{{ scope.row.T1.text }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column
                            prop="T2"
                        ...
                        >
                            <template slot-scope="scope">
                                <div
                                    v-if="
                                        scope.row.T2.text.indexOf('确认人') >= 0
                                    "
                                    class="left white"
                                >
                                    {{ scope.row.T2.text }}
                                ...
                                <span
                                    v-else-if="scope.row.T2.red"
                                    class="red"
                                    >{{ scope.row.T2.text }}</span
                                >
                        ...
                        >
                            <template slot-scope="scope">
                                <div v-if="scope.row.T3.red" class="red">
                                    {{ scope.row.T3.text }}
                                </div>
                                <span v-else class="white">{{
                                    scope.row.T3.text
                        ...
                        >
                            <template slot-scope="scope">
                                <div v-if="scope.row.T4.red" class="red">
                                    {{ scope.row.T4.text }}
                                </div>
                                <span v-else class="white">{{
                                    scope.row.T4.text
                        ...
                        >
                            <template slot-scope="scope">
                                <div v-if="scope.row.T5.red" class="red">
                                    {{ scope.row.T5.text }}
                                </div>
                                <span v-else class="white">{{
                                    scope.row.T5.text
...
export default {
    ...
    methods: {
        exportTable() {
            let filterVal = ['seleTime', 'T1', 'T2', 'T3', 'T4', 'T5'];
            let title = ['时间', '', '一号线', '二号线', '三号线', '四号线'];
            let tableList = [];
            this.tableData.map((_) => {
                ...
                tableList.push({ ...item, seleTime: this.seleTime });
            });
            tableList.pop();
            require.ensure([], () => {
                const {
                ...
                this.json2excel(excelDatas, '注料机重量表', true, 'xlsx');
            });
        },
        ...
        GetProcessParamDataByTime() {
            ...
            this.$axios
                .get(`/api/FeedMachineWeightQuery/Query?time=${this.seleTime}`)
                .then((res) => {
                    if (res.data.code == 0) {
                        ...
                        let temp = [
                            {
                                ...
                                T2: {
                                    text: data.TheoreticalWeight1,
                                    red: data.IsMarkTheoreticalWeight1,
                                },
                                T3: {
                                    text: data.TheoreticalWeight2,
                                    red: data.IsMarkTheoreticalWeight2,
                                },
                                T4: {
                                    text: data.TheoreticalWeight3,
                                    red: data.IsMarkTheoreticalWeight3,
                                },
                                T5: {
                                    text: data.TheoreticalWeight4,
                                    red: data.IsMarkTheoreticalWeight4,
                                },
                            },
                            {
                                T1: {
                                ...
                                T2: {
                                    text: data.ActualWeight1,
                                    red: data.IsMarkActualWeight1,
                                },
                                T3: {
                                    text: data.ActualWeight2,
                                    red: data.IsMarkActualWeight2,
                                },
                                T4: {
                                    text: data.ActualWeight3,
                                    red: data.IsMarkActualWeight3,
                                },
                                T5: {
                                    text: data.ActualWeight4,
                                    red: data.IsMarkActualWeight4,
                                },
                            },
                            // {
                            //     T1: {
                            ...
                            //     },
                            //     T2: {
                            //         text: data.SaggerNumber1,
                            //         red: data.IsMarkSaggerNumber1,
                            //     },
                            //     T3: {
                            //         text: data.SaggerNumber2,
                            //         red: data.IsMarkSaggerNumber2,
                            //     },
                            //     T4: {
                            //         text: data.SaggerNumber3,
                            //         red: data.IsMarkSaggerNumber3,
                            //     },
                            //     T5: {
                            //         text: data.SaggerNumber4,
                            //         red: data.IsMarkSaggerNumber4,
                            //     },
                            // },
                            {
                                T1: {
...
<style>
.weightOfInjectionMachine-Page .el-table--border td,
.weightOfInjectionMachine-Page .el-table--border th,
.weightOfInjectionMachine-Page
...
.weightOfInjectionMachine-Page .el-table td,
.el-table th.is-leaf {
    border-bottom: 1px solid #304171;
}
.weightOfInjectionMachine-Page .el-table--border th.gutter:last-of-type {
    background: #1e304a;
}
.weightOfInjectionMachine-Page
